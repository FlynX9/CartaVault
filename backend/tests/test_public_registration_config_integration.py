...
def test_public_registration_is_disabled_without_explicit_instance_setting(integration_client, database_session) -> None:
    ...
    assert integration_client.get("/auth/registration-status").json() == {"enabled": False}
    response = integration_client.post("/auth/register", json={"email": "visitor@example.test", "password": "a sufficiently long password", "confirmation": "a sufficiently long password"})
...
def test_admin_can_enable_registration_but_regular_users_cannot(integration_client, database_session) -> None:
    ...
    assert integration_client.get("/auth/registration-status").json() == {"enabled": True}
