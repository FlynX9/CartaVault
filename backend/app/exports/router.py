...
def create_export(map_id: UUID, options: KmzExportOptions, session: Session = Depends(get_db)) -> KmzExportCreated:
    item, report = create_kmz_export(session, map_id, options)
...
def download_export(map_id: UUID, export_id: UUID) -> FileResponse:
    item = get(export_id, map_id)
    if item is None:
