...
# Some ISO territories are included in a larger official Geofabrik extract.
# Give them their own output slug and generation bounds so the resulting
# archive remains territory-sized.
_GEOFABRIK_ALIASES: dict[str, tuple[str, str, tuple[float, float, float, float]]] = {
    "AX": ("europe/finland-latest.osm.pbf", "aland", (19.0, 59.65, 21.2, 60.75)),
}
...
VECTOR_COUNTRY_CATALOG = {
    str(country["iso_alpha2"]): VectorCountrySource(
        ...
        slug=_GEOFABRIK_ALIASES[str(country["iso_alpha2"])][1] if str(country["iso_alpha2"]) in _GEOFABRIK_ALIASES else _source_slug(str(country["iso_alpha2"]), _GEOFABRIK_PATHS.get(str(country["iso_alpha2"])), _PATH_COUNTS),
        geofabrik_path=_GEOFABRIK_ALIASES[str(country["iso_alpha2"])][0] if str(country["iso_alpha2"]) in _GEOFABRIK_ALIASES else _GEOFABRIK_PATHS.get(str(country["iso_alpha2"])),
        bounds=_GEOFABRIK_ALIASES[str(country["iso_alpha2"])][2] if str(country["iso_alpha2"]) in _GEOFABRIK_ALIASES else None,
    )
