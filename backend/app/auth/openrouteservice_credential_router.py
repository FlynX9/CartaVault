...
from app.config import openroute_service_settings
...
from app.trips.routing.registry import ors_routing_rate_limiter


router = APIRouter(prefix="/account/integrations/openrouteservice", tags=["account"])
...
def _status(credential: UserApiCredential | None) -> dict[str, object]:
    return {
        ...
        "self_hosted": openroute_service_settings.allow_unauthenticated,
    }
...
async def store_credential(request: Request, session: Session = Depends(get_db), current: UserSession = Depends(get_current_session)) -> dict[str, object]:
    ...
    if not isinstance(value, str):
        raise HTTPException(422, {"code": "CREDENTIAL_VALUE_INVALID", "message": "Une clé OpenRouteService est requise."})
    api_key = value.strip()
    if not api_key or len(api_key) > MAX_API_KEY_LENGTH or any(ord(character) < 33 or ord(character) > 126 for character in api_key):
        raise HTTPException(422, {"code": "CREDENTIAL_VALUE_INVALID", "message": "La clé OpenRouteService fournie est invalide."})
    try:
...
def verify_credential(session: Session = Depends(get_db), current: UserSession = Depends(get_current_session)) -> dict[str, object]:
    credential = _credential(session, current.user_id)
    if credential is None:
        raise HTTPException(404, {"code": "ROUTING_CREDENTIAL_NOT_CONFIGURED", "message": "Aucune clé OpenRouteService n’est configurée."})
    try:
        api_key = CredentialEncryptionService.from_settings().decrypt(credential.encrypted_secret, credential.encryption_version)
        ors_routing_rate_limiter.check(f"verify:ors:{current.user_id}")
        OpenRouteServiceProvider(api_key, open_route_settings_for_verification()).calculate_route(VERIFY_COORDINATES)
    except CredentialEncryptionError as error:
        ...
        session.commit()
        status_code = 429 if error.code in {"ORS_QUOTA_EXCEEDED", "GOOGLE_ROUTING_RATE_LIMITED"} else 503 if error.code in {"ORS_TIMEOUT", "ORS_PROVIDER_ERROR", "ORS_INVALID_RESPONSE"} else 422
...
def open_route_settings_for_verification():
    return openroute_service_settings
...
async def remove_credential(request: Request, session: Session = Depends(get_db), current: UserSession = Depends(get_current_session)) -> dict[str, object]:
    ...
    provider_reset = preferences["routing"]["provider"] == "openrouteservice" and not openroute_service_settings.allow_unauthenticated
    if provider_reset:
