...
from app.auth.sensitive_auth_rate_limit import reserve_email_mfa_send, verify_current_password
...
def _send_code(session: Session, current: UserSession, purpose: str) -> str:
    ...
    verify_current_password(session, current, purpose, password_verifier=verify_password)
    reserve_email_mfa_send(session, current)
    ...
    try:
        ...
        raise HTTPException(503, "Le facteur e-mail est indisponible.") from error
    record_auth_event(session, "email_mfa_enrollment_started", "accepted", actor_user_id=current.user_id); session.commit()
    return raw
...
def disable(payload: dict, session: Session = Depends(get_db), current: UserSession = Depends(get_current_session)) -> None:
    verify_current_password(session, current, str(payload.get("current_password") or ""), password_verifier=verify_password)
