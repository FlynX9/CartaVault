...
class TripNight(Base):
    ...
    description: Mapped[str | None] = mapped_column(Text)
    notes: Mapped[str | None] = mapped_column(Text)
    check_in_from_time: Mapped[time | None] = mapped_column(Time)
    check_in_until_time: Mapped[time | None] = mapped_column(Time)
    check_out_from_time: Mapped[time | None] = mapped_column(Time)
    check_out_until_time: Mapped[time | None] = mapped_column(Time)
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, server_default=func.now(), onupdate=func.now())
