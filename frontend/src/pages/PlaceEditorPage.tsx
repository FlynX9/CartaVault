...
import { addPlaceCategory, addPlaceTag, createPlace, deletePlace, getPlaceDetails, movePlace, refreshPlaceRegion, removePlaceCategory, removePlaceTag, replacePlaceLinks, restorePlace, setPrimaryPlaceCategory, updatePlace } from '../api/places'
...
import { MovePlaceDialog } from '../components/places/MovePlaceDialog'
...
import { allowNextUnsavedNavigation } from '../hooks/useUnsavedChangeSignal'
...
async function syncLinks(placeId: string, initial: PlaceFormValues, current: PlaceFormValues) {
  const comparable = (values: PlaceFormValues) => values.links.map((link) => ({ id: link.id, label: link.label.trim(), url: link.url.trim() }))
  if (JSON.stringify(comparable(initial)) === JSON.stringify(comparable(current))) return
  await replacePlaceLinks(placeId, current.links.map((link, sort_order) => ({ id: link.id, label: link.label.trim(), url: link.url.trim(), sort_order })))
}
...
export function PlaceEditorPage({ mode, placeId: providedPlaceId, embedded = false, activeMapId = null, activeStatusId = null, maps, onPlaceMutated, geographicPrefill = null, coordinatePrefill = null, draftPosition = null, onDraftPositionChange = () => undefined }: Props) {
  ...
  const [moveBlockers, setMoveBlockers] = useState<{ categories: number; tags: number; annotations: number; trip_stops: number; trip_nights: number; trip_anchors: number } | null>(null)
  ...
  const submit = async (values: PlaceFormValues) => {
    ...
    try {
      ...
      await syncLinks(savedId, initialValues, values)
      ...
      if (mode === 'create') {
        ...
        recordReversibleAction({
          ...
          undo: async () => { if (Object.keys(inversePayload).length) await updatePlace(historyPlaceId, { ...inversePayload, confirm_outside_country: true }); await syncAssociations(historyPlaceId, values, initialValues); await syncLinks(historyPlaceId, values, initialValues); announceWorkspaceChanged() },
          redo: async () => { if (Object.keys(forwardPayload).length) await updatePlace(historyPlaceId, { ...forwardPayload, confirm_outside_country: true }); await syncAssociations(historyPlaceId, initialValues, values); await syncLinks(historyPlaceId, initialValues, values); announceWorkspaceChanged() },
        })
      }
      ...
      allowNextUnsavedNavigation()
      document.dispatchEvent(new CustomEvent('cartavault:poi-editor-unsaved', { detail: { formDirty: false, pendingPhotos: false } }))
  ...
  const submitMove = async (targetMapId: string, targetStatusId: string) => {
    if (!placeId || moveBusy) return
    setMoveBusy(true); setMoveError(null); setMoveBlockers(null)
    try {
      const moved = await movePlace(placeId, { target_map_id: targetMapId, target_status_id: targetStatusId })
      setMoveOpen(false)
      onPlaceMutated({ placeId, mapId: moved.map_id })
      publishGlobalFeedback('success', `POI « ${moved.name} » déplacé.`)
      allowNextUnsavedNavigation()
      document.dispatchEvent(new CustomEvent('cartavault:poi-editor-unsaved', { detail: { formDirty: false, pendingPhotos: false } }))
      navigate(withMap(`/places/${placeId}`, moved.map_id, moved.status.id))
    } catch (caught) {
      if (caught instanceof ApiError && caught.code === 'PLACE_MOVE_BLOCKED' && typeof caught.detail === 'object' && caught.detail !== null && 'blockers' in caught.detail && typeof caught.detail.blockers === 'object' && caught.detail.blockers !== null) setMoveBlockers(caught.detail.blockers as { categories: number; tags: number; annotations: number; trip_stops: number; trip_nights: number; trip_anchors: number })
      setMoveError(caught instanceof Error ? caught.message : 'Déplacement impossible.')
    } finally { setMoveBusy(false) }
  }
  const openMoveDialog = () => {
    setMoveError(null)
    setMoveBlockers(null)
    setMoveOpen(true)
  }
  const requestMoveDialog = async () => {
    if (!formDirty) {
      openMoveDialog()
      return
    }
  ...
  return <><article className={`editor-page${embedded ? ' embedded' : ''}`}>{!embedded && <Link className="back-link" to={withMap('/', activeMapId)}>← Annuler</Link>}<header className="editor-header"><p className="details-kicker">{mode === 'create' ? 'Nouveau point d’intérêt' : 'Modification'}</p><h2>{mode === 'create' ? 'Ajouter un POI' : initialValues.name}</h2>{mode === 'edit' && <button type="button" className="secondary-button cv-action-button" disabled={submitting} onClick={() => void requestMoveDialog()}>Déplacer vers une autre carte</button>}</header><PlaceForm key={`${placeId ?? 'new'}:${formResetVersion}`} initialValues={initialValues} maps={maps} allowMapChange={mode === 'create'} isExistingPlace={mode === 'edit'} categories={categories} tags={tags} statuses={statuses} submitLabel={mode === 'create' ? 'Créer le POI' : 'Enregistrer les modifications'} isSubmitting={submitting} serverErrors={fieldErrors} globalError={error} draftPosition={draftPosition} onDraftPositionChange={onDraftPositionChange} regionMetadata={regionDetails ? { manuallyOverridden: regionDetails.region_manually_overridden === true, resolvedAt: regionDetails.region_resolved_at ?? null } : null} onRefreshRegion={mode === 'edit' ? recalculateRegion : undefined} afterLocation={mode === 'create' ? <PhotoUploader files={pendingPhotos} onChange={setPendingPhotos} disabled={submitting} /> : mode === 'edit' && placeId ? <PhotoGalleryManager placeId={placeId} onChanged={() => onPlaceMutated({ placeId, mapId: initialValues.mapId })} /> : null} onDirtyChange={setFormDirty} onSubmit={submit} /></article>{moveOpen && <MovePlaceDialog maps={maps} sourceMapId={initialValues.mapId} busy={moveBusy} error={moveError} blockers={moveBlockers} onClose={() => { if (!moveBusy) setMoveOpen(false) }} onMove={submitMove} />}{confirmationDialog}</>
}
