...
describe('StatusesPage', () => {
  ...
  it('explains the functional state and confirms an impactful change', async () => {
    ...
    expect(screen.queryByText(/regrouper les lieux dans les filtres/)).not.toBeInTheDocument()
    const helpButtons = screen.getAllByRole('button', { name: 'Afficher l’aide du champ' })
    fireEvent.mouseEnter(helpButtons[0])
    expect(await screen.findByRole('tooltip', { name: /regrouper les lieux dans les filtres/ })).toBeVisible()
    ...
    fireEvent.mouseEnter(helpButtons[1])
    expect(await screen.findByRole('tooltip', { name: /Un statut inactif reste associé/ })).toBeVisible()
    fireEvent.mouseEnter(helpButtons[2])
    expect(await screen.findByRole('tooltip', { name: /automatiquement proposé lors de la création/ })).toBeVisible()
    fireEvent.click(screen.getByRole('radio', { name: 'Visité' }))
    fireEvent.click(screen.getByRole('button', { name: 'Enregistrer' }))
