...
import { Activity, Check, ChevronLeft, ChevronRight, Gauge, ImageDown, KeyRound, RefreshCw, Save, ShieldCheck, Trash2, Users, X } from 'lucide-react'
...
function MediaMaintenancePanel() {
  const [limit, setLimit] = useState(5); const [task, setTask] = useState<{ id: string; status: string; percent: number; message: string | null; result: Record<string, unknown> | null; error: string | null } | null>(null); const [error, setError] = useState<string | null>(null); const [busy, setBusy] = useState(false)
  useEffect(() => { const controller = new AbortController(); void getMediaUploadSettings(controller.signal).then((value) => setLimit(value.max_upload_megabytes)).catch((reason) => { if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : 'Réglage média indisponible.') }); return () => controller.abort() }, [])
  useEffect(() => {
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(null); try { const updated = await saveMediaUploadSettings(limit); setLimit(updated.max_upload_megabytes) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Enregistrement impossible.') } finally { setBusy(false) } }
