...
import { getGoogleSatelliteAdminStatus, saveGoogleSatelliteSettings, type GoogleSatelliteAdminStatus } from '../../api/googleSatellite'
...
const sections = [
  ...
  ['offline-maps', MapPinned, 'admin.sections.offlineMaps'], ['quotas', Gauge, 'admin.sections.quotas'], ['instance', Activity, 'admin.sections.instance'],
] as const
  ...
  return createPortal(<div className="account-overlay admin-console-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) requestClose() }}>
    <section ref={modal} className="admin-console" role="dialog" aria-modal="true" aria-labelledby="admin-console-title">
      ...
      <AdminSaveContext.Provider value={saveContext}><div className="admin-console__content">
        ...
        {visitedSections.has('offline-maps') && <div hidden={activeSection !== 'offline-maps'}><OfflineMapsSection /></div>}
...
function OfflineMapsSection() {
  const { t } = useI18n()
  return <section><SectionHeading eyebrow={t('admin.offlineMaps.eyebrow')} title={t('admin.offlineMaps.title')} description={t('admin.offlineMaps.description')} /><VectorBasemapSettingsPanel /></section>
}
...
function VectorBasemapSettingsPanel() {
  ...
  useEffect(() => { const controller = new AbortController(); void load(controller.signal).catch((reason) => { if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : 'Données cartographiques hors ligne indisponibles.') }); return () => controller.abort() }, [load])
  ...
  useAdminSaveEntry('offline-maps-vector-basemaps', useMemo(() => ({ label: 'données cartographiques hors ligne', dirty, busy, save, discard: () => setSettings(saved) }), [busy, dirty, save, saved]))
...
export function GoogleSatelliteAdminPanel() {
  ...
  return <section className="admin-console__card admin-console__setting-card" aria-labelledby="google-satellite-title"><header className="admin-console__setting-header"><span className="admin-console__setting-icon"><KeyRound size={17} /></span><div><h3 id="google-satellite-title">Google Satellite</h3><p>Option BYOK chargée directement dans le navigateur via Maps JavaScript API.</p></div><span className={`admin-console__setting-status ${status?.settings.maps_javascript_enabled ? 'ok' : 'warning'}`}>{status?.settings.maps_javascript_enabled ? 'Actif' : 'Inactif'}</span></header>
    {error && <div className="form-alert" role="alert">{error}</div>}
      {status && <><form className="admin-console__setting-grid" onSubmit={saveSettings}><label className="admin-console__setting-check"><span>Autoriser Google Satellite via Maps JavaScript API</span><input type="checkbox" checked={status.settings.maps_javascript_enabled} onChange={(event) => setStatus({ ...status, settings: { maps_javascript_enabled: event.target.checked } })} /></label><div className="admin-console__setting-actions"><button className="primary-button" disabled={busy}><Save size={16} />Enregistrer</button></div></form>
      <p className="admin-console__hint">Les images sont chargées du navigateur vers Google. CartaVault ne proxyfie et ne compte aucune tuile.</p></>}
  </section>
}
