...
export function GoogleSatelliteAdminPanel() {
  ...
  return <section className="admin-console__card admin-console__setting-card" aria-labelledby="google-satellite-title"><header className="admin-console__setting-header"><span className="admin-console__setting-icon"><KeyRound size={17} /></span><div><h3 id="google-satellite-title">Google Satellite</h3><p>Deux intégrations indépendantes : Maps JavaScript et Map Tiles.</p></div><span className={`admin-console__setting-status ${status?.settings.maps_javascript_enabled || status?.settings.enabled ? 'ok' : 'warning'}`}>{status?.settings.maps_javascript_enabled || status?.settings.enabled ? 'Actif' : 'Inactif'}</span></header>
    {error && <div className="form-alert" role="alert">{error}</div>}
      ...
      <form className="admin-console__setting-grid" onSubmit={saveSettings}><label className="admin-console__setting-check"><span>Autoriser Google Satellite via Maps JavaScript API</span><input type="checkbox" checked={status.settings.maps_javascript_enabled ?? false} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, maps_javascript_enabled: event.target.checked } })} /></label><label className="admin-console__setting-check"><span>Autoriser Google Satellite via Map Tiles API</span><input type="checkbox" checked={status.settings.enabled} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, enabled: event.target.checked } })} /></label><label>Seuil journalier Map Tiles<input type="number" min="100" value={status.settings.daily_soft_limit} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, daily_soft_limit: Number(event.target.value) } })} /></label><label>Seuil mensuel Map Tiles<input type="number" min="100" value={status.settings.monthly_soft_limit} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, monthly_soft_limit: Number(event.target.value) } })} /></label><label>Désactivation Map Tiles à (%)<input type="number" min="50" max="200" value={status.settings.auto_disable_percent} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, auto_disable_percent: Number(event.target.value) } })} /></label><div className="admin-console__setting-actions"><button className="primary-button" disabled={busy}><Save size={16} />Enregistrer</button><button type="button" disabled={busy} onClick={() => void run(resetGoogleSatelliteErrors)}><RefreshCw size={16} />Réinitialiser les erreurs Map Tiles</button></div></form>
