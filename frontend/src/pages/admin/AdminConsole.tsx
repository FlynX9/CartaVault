...
import { deleteGoogleSatelliteCredential, getGoogleSatelliteAdminStatus, resetGoogleSatelliteErrors, saveGoogleSatelliteCredential, saveGoogleSatelliteSettings, verifyGoogleSatelliteCredential, type GoogleSatelliteAdminStatus } from '../../api/googleSatellite'
...
function AdminUsersSection() {
  ...
  return <section>
    ...
    <section className="admin-console__card admin-console__public-registration"><h3>Inscriptions publiques</h3><p>Par défaut, aucune demande extérieure n’est acceptée. Chaque demande reçue reste inactive jusqu’à sa validation par un administrateur.</p><label className="trip-view-toggle"><input type="checkbox" checked={publicRegistrationEnabled} disabled={savingPublicRegistration} onChange={() => void togglePublicRegistration()} /><i aria-hidden="true" /><span>{publicRegistrationEnabled ? 'Inscriptions autorisées' : 'Inscriptions désactivées'}</span></label></section>
...
function GoogleSatelliteAdminPanel() {
  ...
  const saveKey = (event: FormEvent) => { event.preventDefault(); void run(async () => { const next = await saveGoogleSatelliteCredential(key); setKey(''); return next }) }
  const saveSettings = (event: FormEvent) => { event.preventDefault(); if (status) void run(() => saveGoogleSatelliteSettings(status.settings)) }
  return <section className="admin-console__card" aria-labelledby="google-satellite-title"><header><KeyRound size={19} /><div><h3 id="google-satellite-title">Google Satellite</h3><p>Clé navigateur restreinte à Map Tiles API et aux domaines CartaVault.</p></div><span className={status?.available ? 'ok' : 'warning'}>{status?.available ? 'Actif' : 'Inactif'}</span></header>
    {error && <div className="form-alert" role="alert">{error}</div>}
    {status && <><dl><dt>Clé</dt><dd>{status.masked_value ?? 'Non configurée'}</dd><dt>Vérification</dt><dd>{status.verified ? 'Vérifiée' : 'À vérifier'}</dd><dt>Tuiles aujourd’hui</dt><dd>{status.usage.tiles_started_today.toLocaleString('fr-FR')}</dd><dt>Tuiles ce mois</dt><dd>{status.usage.tiles_started_month.toLocaleString('fr-FR')}</dd><dt>Échecs aujourd’hui</dt><dd>{status.usage.tiles_failed_today.toLocaleString('fr-FR')}</dd><dt>Alerte locale</dt><dd>{status.warning_level ? `${status.warning_level} %` : 'Aucune'}</dd><dt>Dernière erreur</dt><dd>{status.last_error_code ?? status.settings.disabled_reason ?? 'Aucune'}</dd></dl>
      <form className="admin-console__credential-form" onSubmit={saveKey}><label>Clé Map Tiles API<input type="password" value={key} required autoComplete="off" placeholder="AIza••••••••" onChange={(event) => setKey(event.target.value)} /></label><button className="primary-button" disabled={busy}><Save size={16} />{status.configured ? 'Remplacer' : 'Enregistrer'}</button>{status.configured && <><button type="button" disabled={busy} onClick={() => void run(verifyGoogleSatelliteCredential)}><RefreshCw size={16} />Vérifier</button><button className="danger" type="button" disabled={busy} onClick={() => void deleteGoogleSatelliteCredential().then(() => load())}><Trash2 size={16} />Supprimer</button></>}</form>
      <form className="admin-console__quota-form" onSubmit={saveSettings}><label>Activer<input type="checkbox" checked={status.settings.enabled} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, enabled: event.target.checked } })} /></label><label>Seuil journalier<input type="number" min="100" value={status.settings.daily_soft_limit} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, daily_soft_limit: Number(event.target.value) } })} /></label><label>Seuil mensuel<input type="number" min="100" value={status.settings.monthly_soft_limit} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, monthly_soft_limit: Number(event.target.value) } })} /></label><label>Désactivation à (%)<input type="number" min="50" max="200" value={status.settings.auto_disable_percent} onChange={(event) => setStatus({ ...status, settings: { ...status.settings, auto_disable_percent: Number(event.target.value) } })} /></label><button className="primary-button" disabled={busy}><Save size={16} />Enregistrer</button><button type="button" disabled={busy} onClick={() => void run(resetGoogleSatelliteErrors)}><RefreshCw size={16} />Réinitialiser les erreurs</button></form>
