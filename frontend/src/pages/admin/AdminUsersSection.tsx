...
import { Check, ChevronLeft, ChevronRight, CircleCheck, Ellipsis, History, LockKeyhole, Mail, MonitorSmartphone, MoreHorizontal, Search, Shield, ShieldCheck, SlidersHorizontal, UserRound, X } from 'lucide-react'
...
export function AdminUsersSection() {
  ...
  const load = useCallback((signal?: AbortSignal) => {
    setLoading(true)
    void Promise.all([getAdminUsers({ q: query.trim(), role, state, page }, signal), getQuotaProfiles(signal)])
      .then(([users, nextProfiles]) => { if (!signal?.aborted) { setResult(users); setProfiles(nextProfiles) } })
