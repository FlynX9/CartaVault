...
import { FloatingPanelWindow } from '../components/layout/FloatingPanelWindow'
...
const TRIP_PANEL_MIN_WIDTH = 608
...
export function MapPage({
  ...
  return (
    <section
      ref={workspaceRef}
      className={`map-workspace${placeListOpen || tripPlanningActive || sidebarOpen ? ' desktop-floating-panels' : ''}${placeListOpen ? ' place-list-open' : ''}${sidebarOpen ? ' sidebar-open' : ''}${tripPlanningActive ? ' trip-planning-open' : ''}${tripPlannerCollapsed ? ' trip-planner-collapsed' : ''}${mobilePlaceDetailOpen ? ' mobile-place-detail-open' : ''}`}
    ...
    >
      ...
      {tripPlanningActive ? <FloatingPanelWindow kind="trips" label="Panneau Sortie" storageKey={TRIPS_WINDOW_KEY} initialGeometry={tripsWindowInitialGeometry} minWidth={TRIP_PANEL_MIN_WIDTH} defaultMode="floating" resetVersion={tripsPanelResetVersion} active={activeFloatingPanel === 'trips'} hidden={!sidebarOpen || tripViewOnly} onActivate={() => setActiveFloatingPanel('trips')} onGeometryCommit={(next) => { setRightPanelWidth(next.width); savePanelWidth(RIGHT_PANEL_WIDTH_KEY, next.width) }}>{sidebar}</FloatingPanelWindow> : sidebarOpen ? <FloatingPanelWindow kind="editor" label="Éditeur du lieu" storageKey={PLACE_EDITOR_WINDOW_KEY} initialGeometry={editorWindowInitialGeometry} minWidth={380} minHeight={360} defaultMode="floating" resetVersion={floatingPanelResetVersion} active={activeFloatingPanel === 'editor'} onActivate={() => setActiveFloatingPanel('editor')} onDockedWidthChange={(width) => { setRightPanelWidth(width); savePanelWidth(RIGHT_PANEL_WIDTH_KEY, width) }} onGeometryCommit={(next) => { setRightPanelWidth(next.width); savePanelWidth(RIGHT_PANEL_WIDTH_KEY, next.width) }}>{sidebar}</FloatingPanelWindow> : sidebar}
