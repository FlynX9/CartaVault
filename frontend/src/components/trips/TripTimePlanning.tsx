...
export function DayTimingSettings({ day, summary: _summary, canEdit, busy, endsAtHotel = false, onSave }: DayTimingProps) {
  ...
  return <section className="trip-time-settings">
    ...
    <div className="trip-time-settings__grid">
      <label><span>{endsAtHotel ? 'Arrivée souhaitée à l’hôtel' : 'Heure cible de fin de journée'}<FieldHelp>CartaVault utilisera cette heure pour proposer une heure de départ.</FieldHelp></span><input aria-label={endsAtHotel ? 'Arrivée souhaitée à l’hôtel' : 'Heure cible de fin de journée'} type="time" value={draft.target_arrival_time ?? ''} disabled={!canEdit || busy} onChange={(event) => update('target_arrival_time', event.target.value || null)} /></label>
      <label><span>Temps tampon entre les étapes<FieldHelp>Temps ajouté entre deux visites pour le stationnement, la préparation et les petits retards.</FieldHelp></span><span className="trip-time-settings__combined"><select aria-label="Préréglage du temps tampon" value={bufferPresets.includes(draft.default_stop_buffer_minutes) ? draft.default_stop_buffer_minutes : 'custom'} disabled={!canEdit || busy} onChange={(event) => { if (event.target.value !== 'custom') update('default_stop_buffer_minutes', Number(event.target.value)) }}>{bufferPresets.map((minutes) => <option key={minutes} value={minutes}>{minutes} min</option>)}<option value="custom">Personnalisé</option></select>{!bufferPresets.includes(draft.default_stop_buffer_minutes) && <input aria-label="Temps tampon personnalisé" type="number" min="0" max="720" step="5" value={draft.default_stop_buffer_minutes} disabled={!canEdit || busy} onChange={(event) => update('default_stop_buffer_minutes', Number(event.target.value))} />}</span></label>
...
export function TripLoadSettingsForm({ trip, canEdit, busy, onSave, value, onChange, embedded = false }: { trip: Trip; canEdit: boolean; busy: boolean; onSave?: (settings: TripLoadSettings) => Promise<void>; value?: TripLoadSettings; onChange?: (settings: TripLoadSettings) => void; embedded?: boolean }) {
  ...
  const fields = <><div className="trip-load-thresholds"><label>Charge faible jusqu’à (min)<input type="number" min="1" max="1440" value={current.low_load_max_minutes} disabled={!canEdit || busy} onChange={(event) => update('low_load_max_minutes', Number(event.target.value))} /></label><label>Charge modérée jusqu’à (min)<input type="number" min="1" max="2880" value={current.medium_load_max_minutes} disabled={!canEdit || busy} onChange={(event) => update('medium_load_max_minutes', Number(event.target.value))} /></label></div><div className="trip-load-colors"><label><span>Couleur faible</span><input type="color" value={current.low_load_color} disabled={!canEdit || busy} onChange={(event) => update('low_load_color', event.target.value.toUpperCase())} /></label><label><span>Couleur modérée</span><input type="color" value={current.medium_load_color} disabled={!canEdit || busy} onChange={(event) => update('medium_load_color', event.target.value.toUpperCase())} /></label><label><span>Couleur élevée</span><input type="color" value={current.high_load_color} disabled={!canEdit || busy} onChange={(event) => update('high_load_color', event.target.value.toUpperCase())} /></label></div><div className="trip-load-preview" aria-label="Prévisualisation des niveaux de charge"><span className="trip-load-preview-badge" style={{ '--trip-load-color': current.low_load_color } as CSSProperties}><Gauge aria-hidden="true" size={12} /><strong>Faible</strong></span><span className="trip-load-preview-badge" style={{ '--trip-load-color': current.medium_load_color } as CSSProperties}><Gauge aria-hidden="true" size={12} /><strong>Modérée</strong></span><span className="trip-load-preview-badge" style={{ '--trip-load-color': current.high_load_color } as CSSProperties}><Gauge aria-hidden="true" size={12} /><strong>Élevée</strong></span></div></>
