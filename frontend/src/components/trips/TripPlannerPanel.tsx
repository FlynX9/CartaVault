...
function TripSettings({ trip, canEdit, canDelete, busy, draftName, dirty, loadSettings, routingProviderLabel, countryConstraintName, onNameChange, onStartDateChange, onLoadSettingsChange, onSave, onDuplicate, onDelete }: { trip: Trip; canEdit: boolean; canDelete: boolean; busy: boolean; draftName: string; dirty: boolean; loadSettings: TripLoadSettings; routingProviderLabel: string; countryConstraintName: string | null; onNameChange: (value: string) => void; onStartDateChange: (value: string | null) => void; onLoadSettingsChange: (settings: TripLoadSettings) => void; onSave: () => void; onDuplicate: () => void; onDelete: () => void }) {
  return <section className="trip-panel-section trip-panel-settings" aria-labelledby="trip-settings-title"><header className="trip-panel-settings__header"><span id="trip-settings-title">Paramètres du voyage</span></header><section className="trip-panel-options"><h3>Nom du voyage</h3><div className="trip-panel-fields"><input aria-label="Nom du voyage" value={draftName} readOnly={!canEdit} onChange={(event) => onNameChange(event.target.value)} /><div className="trip-panel-field-meta"><span className={dirty ? 'dirty' : ''}>{dirty ? 'Non enregistré' : 'Enregistré'}</span><span className="trip-panel-inline-actions"><button type="button" aria-label="Dupliquer le voyage" onClick={onDuplicate}><Copy size={13} /></button>{canDelete && <button type="button" aria-label="Supprimer le voyage" onClick={onDelete}><Trash2 size={13} /></button>}</span></div></div></section><section className="trip-settings-dates" aria-label="Dates du voyage"><h3>Dates du voyage</h3><div><label><CalendarDays aria-hidden="true" size={16} /><span><small>Date de départ</small>{canEdit ? <input aria-label="Date de départ du voyage" type="date" value={trip.start_date ?? ''} disabled={busy} onChange={(event) => onStartDateChange(event.target.value || null)} /> : <strong>{formatTripDate(trip.start_date)}</strong>}</span></label><div><Flag aria-hidden="true" size={16} /><span><small>Date d’arrivée</small><strong>{formatTripDate(trip.end_date)}</strong></span></div></div></section><section className="trip-panel-routing-settings" aria-label="Paramètres de routage"><h3>Routage</h3><p><strong>Moteur</strong><span>{routingProviderLabel}</span></p>{countryConstraintName && <p><strong>Contrainte</strong><span>Itinéraire limité à la {countryConstraintName}</span></p>}</section><TripLoadSettingsForm trip={trip} canEdit={canEdit} busy={busy} value={loadSettings} onChange={onLoadSettingsChange} embedded />{canEdit && <div className="trip-panel-settings-actions"><button className="primary" type="button" disabled={busy || loadSettings.low_load_max_minutes >= loadSettings.medium_load_max_minutes} onClick={onSave}><Save size={13} />Enregistrer</button></div>}</section>
}
