...
import { AlertTriangle, Archive, ArchiveRestore, ArrowDown, ArrowUp, Car, Check, ChevronDown, Clock3, Copy, Download, Eye, EyeOff, Flag, GripVertical, Lock, MapPin, Minus, Moon, Navigation, Pencil, Plus, Route, Save, SlidersHorizontal, Sun, Trash2, X } from 'lucide-react'
    ...
    >
      ...
      <div className="trip-night-content">
        <div className="trip-night-header-row">
          <span className="trip-panel-timeline-title trip-night-title"><strong>Nuit {previous.day_number}</strong><TimelineReadinessIndicator state={night ? 'ready' : 'empty'} label={night ? `Nuit ${previous.day_number} configurée` : `Nuit ${previous.day_number} sans lieu associé`} /></span>
...
type TimelineReadinessState = 'ready' | 'empty' | 'warning'

function getDayReadiness(day: TripDay, summary: TripDayTimeSummary | undefined): { state: TimelineReadinessState; label: string } {
  if (day.stops.length === 0) return { state: 'empty', label: `Jour ${day.day_number} sans étape associée` }
  ...
  if (day.route_status !== 'ready' || summary?.route_is_stale || constraintNeedsAttention) {
    return { state: 'warning', label: `Jour ${day.day_number} : itinéraire à calculer ou vérifier` }
  }
  return { state: 'ready', label: `Jour ${day.day_number} prêt` }
}

function TimelineReadinessIndicator({ state, label }: { state: TimelineReadinessState; label: string }) {
  return <span className={`trip-timeline-readiness trip-timeline-readiness--${state}`} role="img" aria-label={label} title={label}>
    {state === 'ready' ? <Check aria-hidden="true" size={9} /> : state === 'warning' ? <AlertTriangle aria-hidden="true" size={9} /> : <X aria-hidden="true" size={9} />}
  </span>
}
