...
export function TripPlannerPanel({ poiMap, trip, activeDayId, tripViewOnly = false, hiddenDayIds = new Set<string>(), onTripViewOnlyChange = () => undefined, onDayVisibilityChange = () => undefined, onTripChange, onActiveDayChange, onStopFocus, onClose }: Props) {
  ...
  return <aside className={`map-sidebar trip-planner-panel${tripViewOnly ? ' trip-planner-panel--trip-view' : ''}`} aria-label="Préparation de sortie">
    <header className="trip-panel-header"><div className="trip-panel-heading"><Route aria-hidden="true" size={28} /><div><p className="cv-workspace-panel__eyebrow">Sortie</p><h2 title={trip?.name ?? 'Préparation'}>{trip?.name ?? 'Préparation'}</h2></div></div><div className="trip-panel-header-actions"><button className={`panel-icon-button trip-view-button${tripViewOnly ? ' active' : ''}`} type="button" aria-label={tripViewOnly ? 'Quitter la vue du voyage' : 'Activer la vue du voyage'} aria-pressed={tripViewOnly} title={tripViewOnly ? 'Afficher la préparation complète' : 'Afficher uniquement le voyage'} onClick={() => onTripViewOnlyChange(!tripViewOnly)}><Route size={16} /></button>{trip && canEdit && <button className="panel-icon-button" type="button" aria-label="Dupliquer cette sortie" onClick={() => void run(async () => { const copy = await duplicateTrip(trip.id); await reload(copy.id) })}><Copy size={16} /></button>}<button className="panel-icon-button" type="button" aria-label="Fermer le panneau Sortie" onClick={onClose}><X size={17} /></button></div></header>
    ...
    {trip && <div className="trip-panel-primary-actions"><button type="button" aria-label="Optimiser le voyage" disabled={busy || !activeDayId || (trip.days.find((day) => day.id === activeDayId)?.stops.length ?? 0) < 2} onClick={() => { const day = trip.days.find((item) => item.id === activeDayId); if (day) void run(async () => setOptimization({ dayId: day.id, value: await optimizeTripDay(day.id) })) }}><Navigation size={14} />Optimiser</button><button type="button" onClick={() => void run(() => exportFile('gpx'))}><Download size={14} />Exporter GPX</button><button type="button" aria-label="Afficher les paramètres du voyage" aria-expanded={settingsOpen} onClick={() => setSettingsOpen((current) => !current)}><SlidersHorizontal size={14} /></button></div>}
    ...
    {!trip ? <div className="trip-panel-empty"><Route size={28} /><strong>Aucune sortie préparée</strong><span>Créez un voyage puis ajoutez les POI depuis le panneau Lieux.</span></div> : <>
      ...
      <details className="trip-panel-section trip-panel-journeys" open><summary><span>Trajets</span><small>{trip.days.length} {trip.days.length > 1 ? 'journées' : 'journée'}</small><ChevronDown className="trip-panel-chevron" size={15} /></summary>
        <div className="trip-panel-days">{trip.days.map((day, dayIndex) => <div key={day.id} style={{ '--trip-day-color': day.color ?? '#0FA68A' } as CSSProperties}>
          {dayIndex === 0 && <Departure trip={trip} recommendedStart={daySummaries[day.id]?.recommended_start_time ?? null} recommendedStartOffset={daySummaries[day.id]?.recommended_start_day_offset ?? null} canEdit={canEdit} run={run} reload={reload} />}
          {dayIndex > 0 && <Night previous={trip.days[dayIndex - 1]} next={day} recommendedStart={daySummaries[day.id]?.recommended_start_time ?? null} recommendedStartOffset={daySummaries[day.id]?.recommended_start_day_offset ?? null} trip={trip} canEdit={canEdit} run={run} reload={reload} />}
...
function DayColorPicker({ day, disabled, onChange }: { day: TripDay; disabled: boolean; onChange: (color: string) => void }) {
  return <label className="trip-day-color-picker"><span>Choisir la couleur du tracé et des étapes</span><input aria-label={`Couleur du jour ${day.day_number}`} type="color" value={day.color ?? '#0FA68A'} disabled={disabled} onChange={(event) => onChange(event.target.value.toUpperCase())} /></label>
}

function DaySettings({ day, summary, canEdit, busy, endsAtHotel, onTimingSave, onColorChange }: { day: TripDay; summary: TripDayTimeSummary | undefined; canEdit: boolean; busy: boolean; endsAtHotel: boolean; onTimingSave: (payload: TripDayTimingPayload) => Promise<void>; onColorChange: (color: string) => void }) {
  return <details className="trip-day-settings">
    ...
    <div className="trip-day-settings__body">
      ...
      <section className="trip-day-color-section"><h4>Couleur du jour</h4><DayColorPicker day={day} disabled={!canEdit || busy} onChange={onColorChange} /></section>
    </div>
  </details>
}
...
function Departure({ trip, recommendedStart, recommendedStartOffset, canEdit, run, reload }: { trip: Trip; recommendedStart: string | null; recommendedStartOffset: number | null; canEdit: boolean; run: (action: () => Promise<void>) => Promise<void>; reload: (id?: string) => Promise<void> }) {
...
function Night({ trip, previous, next, recommendedStart, recommendedStartOffset, canEdit, run, reload }: { trip: Trip; previous: TripDay; next: TripDay; recommendedStart: string | null; recommendedStartOffset: number | null; canEdit: boolean; run: (action: () => Promise<void>) => Promise<void>; reload: (id?: string) => Promise<void> }) {
...
function formatNightDuration(checkIn: string | null, checkOut: string | null) { if (!checkIn || !checkOut) return '—'; const [inHours, inMinutes] = checkIn.split(':').map(Number); const [outHours, outMinutes] = checkOut.split(':').map(Number); let duration = outHours * 60 + outMinutes - (inHours * 60 + inMinutes); if (duration < 0) duration += 24 * 60; return formatMinutes(duration) }
