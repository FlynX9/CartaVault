...
interface Props { poiMap: PoiMap; trip: Trip | null; activeDayId: string | null; tripViewOnly?: boolean; onTripViewOnlyChange?: (enabled: boolean) => void; onTripChange: (trip: Trip | null) => void; onActiveDayChange: (id: string | null) => void; onStopFocus?: (latitude: number, longitude: number) => void; onClose: () => void }

export function TripPlannerPanel({ poiMap, trip, activeDayId, tripViewOnly = false, onTripViewOnlyChange = () => undefined, onTripChange, onActiveDayChange, onStopFocus, onClose }: Props) {
  ...
  return <aside className="map-sidebar trip-planner-panel" aria-label="Préparation de sortie">
    ...
    {!trip ? <div className="trip-panel-empty"><Route size={28} /><strong>Aucune sortie préparée</strong><span>Créez un voyage puis ajoutez les POI depuis le panneau Lieux.</span></div> : <>
      ...
      <details className="trip-panel-section trip-panel-journeys" open><summary><span>Trajets</span><small>{trip.days.length} {trip.days.length > 1 ? 'journées' : 'journée'}</small><ChevronDown className="trip-panel-chevron" size={15} /></summary>
        <div className="trip-panel-journeys-actions">{canEdit && <button className="trip-panel-add-day" type="button" onClick={() => void run(async () => { await addTripDay(trip.id); await reload(trip.id) })}><CalendarPlus size={15} />Ajouter une journée</button>}<label className="trip-view-toggle"><span>Vue du voyage</span><input aria-label="Afficher uniquement le voyage" type="checkbox" checked={tripViewOnly} onChange={(event) => onTripViewOnlyChange(event.target.checked)} /><i aria-hidden="true" /></label></div>
