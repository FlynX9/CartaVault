...
interface Props {
  ...
  activeAnchorTarget?: "departure" | "arrival" | null;
  tripViewOnly?: boolean;
  hiddenDayIds?: ReadonlySet<string>;
  collapsed?: boolean;
  createRequest?: number;
  tripSelectorVisible?: boolean;
  restoreCachedState?: boolean;
  onCollapsedChange?: (collapsed: boolean) => void;
  onTripViewOnlyChange?: (enabled: boolean) => void;
  onDayVisibilityChange?: (dayId: string, visible: boolean) => void;
  onTripChange: (trip: Trip | null) => void;
  onActiveDayChange: (id: string | null) => void;
  onActiveAnchorTargetChange?: (target: "departure" | "arrival" | null) => void;
  onActiveNightTargetChange?: (target: TripNightTarget | null, openPopup?: boolean) => void;
  onAnchorPopupChange?: (target: "departure" | "arrival" | null) => void;
  onAnchorPlaceDrop?: (target: "departure" | "arrival", placeId: string) => Promise<void>;
  onStopFocus?: (latitude: number, longitude: number) => void;
  onStopPlaceSelect?: (placeId: string) => void;
...
const TripAnchorActionsContext = createContext<{
  canEdit: boolean;
  reload: (id?: string) => Promise<void>;
  onOpenPopup: (target: "departure" | "arrival") => void;
  onPlaceDrop?: (target: "departure" | "arrival", placeId: string) => Promise<void>;
} | null>(null);
...
type MobileNightDestination = {
  previousDayId: string;
  nextDayId: string;
  label: string;
};
type MobileTimelineBoundaryContextValue = {
  previous: () => void;
  next: () => void;
};
const MobileTimelineBoundaryContext = createContext<MobileTimelineBoundaryContextValue | null>(null);
type MobileDayActionsContextValue = {
  openSettings: (dayId: string) => void;
  openPicker: () => void;
  duplicate: (dayId: string) => void;
  remove: (day: TripDay) => void;
};
const MobileDayActionsContext = createContext<MobileDayActionsContextValue | null>(null);
const tripPanelMetricsCache = new globalThis.Map<
  string,
  {
...
export function TripPlannerPanel({ poiMap, trip, activeDayId, activeAnchorTarget = null, tripViewOnly = false, hiddenDayIds = new Set<string>(), collapsed = false, createRequest = 0, tripSelectorVisible = true, restoreCachedState = false, onCollapsedChange = () => undefined, onTripViewOnlyChange = () => undefined, onDayVisibilityChange = () => undefined, onTripChange, onActiveDayChange, onActiveAnchorTargetChange = () => undefined, onActiveNightTargetChange = () => undefined, onAnchorPopupChange = () => undefined, onAnchorPlaceDrop, onStopFocus, onStopPlaceSelect = () => undefined, onPreviewStopSelect = () => undefined, onPreviewSelectionChange = () => undefined, onUnsavedChangesGuardChange = () => undefined, onInitialLoadComplete = () => undefined, onTechnicalActionsChange }: Props) {
  ...
  const [placeDropDayId, setPlaceDropDayId] = useState<string | null>(null);
  ...
  const [timelineCollapseRequest, setTimelineCollapseRequest] = useState({
    collapsed: false,
    expandedItem: initialExpandedDayId ? `day:${initialExpandedDayId}` : null,
    version: 0,
  });
  const timelineTripIdRef = useRef(trip?.id ?? null);
  ...
  const [mobilePreviewCard, setMobilePreviewCard] = useState<{
    name: string;
    open: () => void;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (trip && timelineTripIdRef.current !== trip.id) {
      timelineTripIdRef.current = trip.id;
      const expandedDayId = trip.days.some((day) => day.id === activeDayId) ? activeDayId : trip.days[0]?.id;
      setCollapsedDayIds(new Set(trip.days.filter((day) => day.id !== expandedDayId).map((day) => day.id)));
      setTimelineCollapseRequest((current) => ({ collapsed: false, expandedItem: expandedDayId ? `day:${expandedDayId}` : null, version: current.version + 1 }));
    } else {
  ...
  const loadTripDetails = useCallback(async (target: string, signal?: AbortSignal) => {
    ...
    return {
      loaded,
      loadedSummary,
      daySummaries: Object.fromEntries(perDay.map((item) => [item.day_id, item])),
    };
  }, []);
  ...
  const selectTrip = useCallback(
    async (target: string) => {
      ...
      if (!target) {
        ...
        setSummary(null);
        setDaySummaries({});
        onTripChangeRef.current(null);
        onActiveDayChangeRef.current(null);
        return;
      }
      const controller = new AbortController();
      loadControllerRef.current = controller;
      setLoadingTripId(target);
      setError(null);
      setSummary(null);
      setDaySummaries({});
      try {
  ...
  const reload = useCallback(
    async (id = trip?.id) => {
      const items = await listTrips(poiMap.id);
      setTrips(items);
    ...
    void listTrips(poiMap.id)
      .then(async (items) => {
        ...
        if (retainedTrip?.map_id === poiMap.id && items.some((item) => item.id === retainedTrip.id)) {
          ...
          const hasCompleteMetrics = retainedMetrics?.summary && retainedTrip.days.every((day) => retainedMetrics.daySummaries[day.id]);
          if (!hasCompleteMetrics) {
            const [loadedSummary, perDay] = await Promise.all([getTripSummary(retainedTrip.id), Promise.all(retainedTrip.days.map((day) => getTripDaySummary(day.id)))]);
            if (!active) return;
            setSummary(loadedSummary);
            setDaySummaries(Object.fromEntries(perDay.map((item) => [item.day_id, item])));
          }
  ...
  const runUndoable = async (label: string, action: () => Promise<void>) => {
    ...
    if (label === "ajout de la journée") {
      const addedDay = after.days.find((day) => !before.days.some((previous) => previous.id === day.id));
      publishGlobalFeedback("success", addedDay ? `Jour ${addedDay.day_number} ajouté à la sortie « ${after.name} ».` : `Journée ajoutée à la sortie « ${after.name} ».`);
    } else if (label === "suppression de la journée") {
      const removedDay = before.days.find((day) => !after.days.some((current) => current.id === day.id));
  ...
  const changeSelectedTrip = (target: string) => {
    ...
    void requestSettingsLeave().then((canLeave) => {
      if (canLeave) void selectTrip(target);
    });
  };
  const toggleSettings = () => {
    ...
    void requestSettingsLeave().then((canLeave) => {
      if (canLeave) setSettingsOpen(false);
    });
  };
  ...
  const insertDayAfter = (day: TripDay) => {
    ...
    void run(() =>
      runUndoable("ajout de la journée", async () => {
        await addTripDay(trip.id, { after_day_id: day.id });
        await reload(trip.id);
      }),
    );
  };
  const removeDay = async (day: TripDay) => {
    if (!trip) return;
  ...
  const drop = (event: DragEvent, day: TripDay) => {
    ...
    if (data.startsWith("place:"))
      void run(() =>
        runUndoable("ajout de l’étape", async () => {
          await addTripStop(day.id, {
            place_id: data.slice(6),
            stop_type: "place",
          });
          await refreshTripSilently(trip!.id);
        }),
    ...
    if (data.startsWith("stop:"))
      void run(() =>
        runUndoable("déplacement de l’étape", async () => {
          await moveTripStop(data.slice(5), day.id, day.stops.length);
          await refreshTripSilently(trip!.id);
        }),
      );
  };
  const dropStop = (event: DragEvent, day: TripDay, index: number) => {
    ...
    if (data.startsWith("place:")) {
      ...
      void run(() =>
        runUndoable("ajout de l’étape", async () => {
          const created = await addTripStop(day.id, {
            place_id: placeId,
            stop_type: "place",
          });
    ...
    void run(() =>
      runUndoable("déplacement de l’étape", async () => {
        await moveTripStop(stopId, day.id, index);
        await refreshTripSilently(trip!.id);
      }),
    );
  };
  const recalculateRoute = (day: TripDay) =>
    void run(async () => {
      if (!canEditTrip) return;
      await calculateTripDayRoute(day.id);
      await reload(trip!.id);
      setRouteFeedback(day.id);
  ...
  const expandTimelineItem = (item: string, dayId?: string) => {
    setCollapsedDayIds(new Set(trip?.days.filter((day) => day.id !== dayId).map((day) => day.id) ?? []));
    setTimelineCollapseRequest((current) => ({ collapsed: false, expandedItem: item, version: current.version + 1 }));
  };
  const toggleDaySettings = (dayId: string) =>
    setOpenDaySettingsIds((current) => {
      const next = new Set(current);
      if (next.has(dayId)) next.delete(dayId);
      else next.add(dayId);
      return next;
    });
  const activateDay = (dayId: string) => {
    setActiveNightTarget(null);
    onActiveNightTargetChange(null);
  ...
  const activateMobileNight = (previousDayId: string, nextDayId: string, direction: MobileDayTransitionDirection) => {
    ...
    mobileMapSyncTimerRef.current = window.setTimeout(() => {
      mobileMapSyncTimerRef.current = null;
      const target = {
        nightId: trip?.nights.find((night) => night.previous_day_id === previousDayId && night.next_day_id === nextDayId)?.id ?? null,
        previousDayId,
        nextDayId,
      };
  ...
  const activateMobileAnchor = (target: "departure" | "arrival", direction: MobileDayTransitionDirection) => {
    const anchor = target === "departure" ? trip?.departure : trip?.arrival;
  ...
  const moveMobileStopToNight = (stop: TripStop, destination: MobileNightDestination) => {
    if (!trip) return;
    void run(() =>
      runUndoable("déplacement de l’étape vers une nuit", async () => {
        ...
        if (existingNight) await updateTripNight(existingNight.id, payload);
          ...
          await addTripNight(trip.id, {
            previous_day_id: destination.previousDayId,
            next_day_id: destination.nextDayId,
            ...payload,
          });
  ...
  const moveMobileNightToDay = (night: Trip["nights"][number], targetDayId: string) => {
    if (!trip) return;
    void run(() =>
      runUndoable("déplacement du lieu de nuit vers une journée", async () => {
        const payload = night.place_id
          ? { place_id: night.place_id, stop_type: "place" }
          : {
              name: night.name,
              latitude: night.latitude,
              longitude: night.longitude,
              address: night.address ?? undefined,
              stop_type: "free_location",
            };
  ...
  const moveMobileNightToNight = (night: Trip["nights"][number], destination: MobileNightDestination) => {
    ...
    void run(() =>
      runUndoable("déplacement du lieu de nuit", async () => {
        ...
        const payload = night.place_id
          ? { place_id: night.place_id, source_type: "place" as const }
          : {
              source_type: "map" as const,
              name: night.name,
              latitude: night.latitude,
  ...
  const mobileNightDestinations: MobileNightDestination[] =
    trip?.days.slice(0, -1).map((day, index) => ({
      previousDayId: day.id,
      nextDayId: trip.days[index + 1].id,
      label: `Nuit ${day.day_number}`,
    })) ?? [];
  ...
  const selectPreviewLocation = (key: string, dayId: string, stopId: string | null) => {
    ...
    setMobilePreviewCard(
      stopId
        ? {
            name: stop?.name ?? "Étape",
            open: () => onPreviewStopSelect(stopId),
          }
        : null,
    );
  };
    ...
    const handleMobileDayAction = (event: Event) => {
      const detail = (
        event as CustomEvent<{
          action: "route" | "optimize" | "add-day" | "create-trip" | "open-picker" | "close-settings";
          dayId?: string;
        }>
      ).detail;
      if (detail?.action === "create-trip") {
        setCreateOpen(true);
        return;
      }
      if (detail?.action === "open-picker") {
        setMobileTripPickerOpen(true);
        return;
      }
      if (detail?.action === "close-settings") {
      ...
      if (detail.action === "optimize" && targetDay.stops.length >= 2)
        void run(
          async () =>
            setOptimization({
              dayId: targetDay.id,
              value: await optimizeTripDay(targetDay.id),
            }),
          `optimize:${targetDay.id}`,
        );
    };
  ...
  const selectPreviewNight = (night: Trip["nights"][number], activateNextDay = true) => {
    ...
    const target = {
      nightId: night.id,
      previousDayId: night.previous_day_id,
      nextDayId: night.next_day_id,
    };
    setActiveNightTarget(target);
    ...
    if (isMobile)
      setMobilePreviewCard({
        name: night.name,
        open: () => onActiveNightTargetChange(target, true),
      });
    else onActiveNightTargetChange(target, true);
  };

  return (
    <MobileTimelineBoundaryContext.Provider
      value={{
        previous: () => activateMobileAnchor("departure", "backward"),
        next: () => activateMobileAnchor("arrival", "forward"),
      }}
    >
      <MobileDayActionsContext.Provider
        value={{
          openSettings: toggleDaySettings,
          openPicker: () => setMobileTripPickerOpen(true),
          duplicate: (dayId) =>
            void run(() =>
              runUndoable("duplication de la journée", async () => {
                await duplicateTripDay(dayId);
                await reload(trip?.id);
              }),
            ),
          remove: (day) =>
            void confirm({
              ...
              message: `Le jour ${day.day_number}, ses étapes et son itinéraire seront définitivement supprimés.`,
            }).then((confirmed) => {
              if (confirmed) void run(() => runUndoable("suppression de la journée", () => removeDay(day)));
            }),
        }}
      >
          ...
          {panelCollapsed ? (
            ...
            <>
              <header className="trip-panel-header places-redesign-header">
                <div>
                  <div className="trip-panel-title-row places-redesign-title-row">
                    <h2>{tripViewOnly ? t("trips.timeline") : t("trips.title")}</h2>
                    {trip && <span className={`places-redesign-count trip-panel-trip-status trip-panel-trip-status--${trip.status === "completed" || trip.status === "archived" ? "completed" : "active"}`}>{trip.status === "completed" || trip.status === "archived" ? t("trips.completed") : t("trips.active")}</span>}
                  </div>
                  {trip && (
                    <p className="trip-panel-header-meta places-redesign-map-meta">
                      <span className="trip-panel-header-name" title={trip.name}>
                        {trip.name}
                      </span>
                      <span aria-hidden="true">·</span>
                      <span className="trip-panel-header-day-count">{t("trips.day", { count: trip.days.length })}</span>
                      {summary && !tripViewOnly && (
                        <span className="trip-panel-mobile-global-metrics" aria-label="Résumé global de la sortie">
                          <span title="Distance totale">
                            <Road aria-hidden="true" size={12} />
                            <strong>{formatRouteDistance(summary.total_route_distance_meters)}</strong>
                          </span>
                          <span title="Temps total">
                            <Clock3 aria-hidden="true" size={12} />
                            <strong>{formatMinutes(summary.total_planned_duration_minutes)}</strong>
                          </span>
                        </span>
                      )}
                    </p>
              ...
              <div className="trip-panel-scroll" role="region" aria-label={t("trips.content")} tabIndex={0}>
                {isMobile && !tripViewOnly && trip && (
                  <MobileTripOverview
                    ...
                    onRoute={recalculateAllRoutes}
                    onOptimize={optimizeAllDays}
                    onTimeline={() => onTripViewOnlyChange(true)}
                    onOffline={() => setOfflineDialogOpen(true)}
                ...
                {isMobile && !tripViewOnly && (
                  <div className="trip-mobile-primary-actions" aria-label="Actions de sortie">
                    ...
                    {canEdit && (
                      <button type="button" className="panel-icon-button primary" title="Ajouter une sortie" aria-label="Ajouter une sortie" onClick={() => setCreateOpen(true)}>
                        <Plus size={17} />
                      </button>
                    )}
                    {trip && (
                      <button type="button" className={`panel-icon-button${settingsOpen ? " active" : ""}`} title="Paramètres de la sortie" aria-label="Paramètres de la sortie" aria-pressed={settingsOpen} onClick={toggleSettings}>
                    ...
                    {trip && canEditTrip && (
                      <button
                        ...
                        onClick={() =>
                          void run(async () => {
                            const created = await addTripDay(trip.id);
                            await reload(trip.id);
                            publishGlobalFeedback("success", `Jour ${created.day_number} ajouté à la sortie « ${trip.name} ».`);
                          })
                        }
                      >
                ...
                {isMobile && !tripViewOnly && trip && mobileDay && (
                  <MobileTripSwipeSurface
                    className={`trip-mobile-day-transition trip-mobile-day-transition--${mobileDayTransitionDirection}${mobileDaySwipeOffset ? " is-swiping" : ""}`}
                    style={
                      {
                        "--trip-mobile-swipe-offset": `${mobileDaySwipeOffset}px`,
                      } as CSSProperties
                    }
                    key={mobileTimelineTarget?.kind === "night" ? `night:${mobileTimelineTarget.previousDayId}:${mobileTimelineTarget.nextDayId}` : mobileDay.id}
                    onPrevious={mobileTimelineTarget?.kind === "night" && mobileNightPreviousDay ? () => activateMobileDay(mobileNightPreviousDay.id, "backward") : () => navigateFromMobileDay(mobileDay, -1)}
                    onNext={mobileTimelineTarget?.kind === "night" && mobileNightNextDay ? () => activateMobileDay(mobileNightNextDay.id, "forward") : () => navigateFromMobileDay(mobileDay, 1)}
                    onSwipeProgress={setMobileDaySwipeOffset}
                  >
                    {mobileTimelineTarget?.kind === "night" && mobileNightPreviousDay && mobileNightNextDay ? (
                      <MobileTripNightNavigator
                        previousDay={mobileNightPreviousDay}
                        nextDay={mobileNightNextDay}
                        night={mobileNight}
                        days={trip.days}
                        nights={mobileNightDestinations}
                        canEdit={canEditTrip}
                        busy={busy}
                        onPrevious={() => activateMobileDay(mobileNightPreviousDay.id, "backward")}
                        onNext={() => activateMobileDay(mobileNightNextDay.id, "forward")}
                        onOpenPopup={() => {
                          if (!mobileNight) return;
                          const target = {
                            nightId: mobileNight.id,
                            previousDayId: mobileNightPreviousDay.id,
                            nextDayId: mobileNightNextDay.id,
                          };
                          setActiveNightTarget(target);
                          onStopFocus?.(mobileNight.latitude, mobileNight.longitude);
                          onActiveNightTargetChange(target, true);
                        }}
                        onDelete={(night) =>
                          void confirm({
                            title: "Supprimer le lieu de nuit ?",
                            message: `« ${night.name} » sera retiré de cette nuit.`,
                          }).then((confirmed) => {
                            if (confirmed)
                              void run(() =>
                                runUndoable("suppression du lieu de nuit", async () => {
                                  await deleteTripNight(night.id);
                                  await refreshTripSilently(trip.id);
                                }),
                              );
                        ...
                        onMoveToDay={moveMobileNightToDay}
                        onMoveToNight={moveMobileNightToNight}
                      />
                    ) : (
                      <>
                        <MobileTripDayNavigator day={mobileDay} days={trip.days} canNavigate onSwipeProgress={setMobileDaySwipeOffset} onPrevious={() => navigateFromMobileDay(mobileDay, -1)} onNext={() => navigateFromMobileDay(mobileDay, 1)} />
                        <MobileTripDayMetrics summary={daySummaries[mobileDay.id] ?? activeDaySummary} />
                        {canEditTrip && (
                          <MobileTripDayCommands
                            day={mobileDay}
                            days={trip.days}
                            nights={mobileNightDestinations}
                            busy={busy}
                            ...
                            onMove={(stopId, targetDayId, targetIndex) =>
                              void run(() =>
                                runUndoable("déplacement de l’étape", async () => {
                                  await moveTripStop(stopId, targetDayId, targetIndex);
                                  await refreshTripSilently(trip.id);
                                  onActiveDayChange(targetDayId);
                                }),
                              )
                            }
                            onMoveToNight={moveMobileStopToNight}
                            onDelete={(stopId, name) =>
                              void confirm({
                                title: "Supprimer cette étape ?",
                                message: `« ${name} » sera retirée de la journée.`,
                              }).then((confirmed) => {
                                if (confirmed)
                                  void run(() =>
                                    runUndoable("suppression de l’étape", async () => {
                ...
                {tripViewOnly ? (
                  <div className="trip-panel-compact-summary">
                    {summary && trip ? (
                      <>
                        <TripPreviewTimeline
                          trip={trip}
                          activeDayId={activeDayId}
                          selectedKey={previewSelectionKey}
                          daySummaries={daySummaries}
                          onSelectDay={(day) => {
                            setMobilePreviewCard(null);
                            setPreviewSelectionKey(`day:${day.id}`);
                            onPreviewStopSelect(null);
                            activateDay(day.id);
                          }}
                          onSelectNight={selectPreviewNight}
                          onSelectLocation={selectPreviewLocation}
                          onNavigateItem={(key, stopId) => {
                            setPreviewSelectionKey(key);
                            if (key.startsWith("night:")) {
                              const night = trip.nights.find((item) => `night:${item.id}` === key);
                              if (night) selectPreviewNight(night, false);
                            } else {
                              setActiveNightTarget(null);
                              onActiveNightTargetChange(null);
                              const stop = trip.days.flatMap((day) => day.stops).find((item) => item.id === stopId);
                              if (isMobile && stopId)
                                setMobilePreviewCard({
                                  name: stop?.name ?? "Étape",
                                  open: () => onPreviewStopSelect(stopId),
                                });
                              else onPreviewStopSelect(stopId);
                            }
                          }}
                        />
                        {isMobile && mobilePreviewCard && (
                          <div className="trip-mobile-preview-card">
                            <span className="trip-mobile-preview-card__image" aria-hidden="true">
                              <MapPin size={16} />
                            </span>
                            <strong>{mobilePreviewCard.name}</strong>
                            <button type="button" onClick={mobilePreviewCard.open}>
                              Ouvrir
                            </button>
                          </div>
                        )}
                      ...
                      <div className="trip-panel-empty" role="status">
                        <Route size={24} />
                        <strong>Chargement du résumé…</strong>
                      </div>
                    )}
                  </div>
                ) : (
                  <>
                    {error && (
                      <p className="trip-panel-error" role="alert">
                        {error === "Internal Server Error" ? "Une erreur serveur empêche cette opération." : error}
                      </p>
                    )}
                    {tripSelectorVisible && <div className="trip-panel-selector">
                      <select aria-label={t("trips.select")} value={loadingTripId ?? trip?.id ?? ""} onChange={(event) => changeSelectedTrip(event.target.value)}>
                        <option value="">{t("trips.select")}</option>
                        {trips.map((item) => (
                          <option key={item.id} value={item.id}>
                            {item.name}
                          </option>
                      ...
                      {canEdit && (
                        <button className="panel-icon-button primary" type="button" aria-label={t("trips.create")} title={t("trips.create")} onClick={() => setCreateOpen(true)}>
                          <Plus size={16} />
                        </button>
                      )}
                    ...
                    {loadingTripId ? (
                      <div className="trip-panel-empty" role="status">
                        <Route size={28} />
                        <strong>{t("trips.loading")}</strong>
                      </div>
                    ) : (
                      <>
                        {!trip ? (
                          <EmptyState
                            className="trip-panel-empty trip-empty-state"
                            icon={<Route size={28} />}
                            title={t("trips.empty.title")}
                            description={isMobile ? undefined : t("trips.empty.description")}
                            action={
                              isMobile && canEdit && trips.length === 0
                                ? {
                                    label: (
                                      <>
                                        <MapPlus size={15} aria-hidden="true" />
                                        {t("trips.create")}
                                      </>
                                    ),
                          ...
                          <>
                            {settingsOpen && activeLoadSettings && (
                              <TripSettings
                                ...
                                stayInCountry={stayInCountryDraft}
                                avoidTolls={avoidTollsDraft}
                                avoidHighways={avoidHighwaysDraft}
                                avoidFerries={avoidFerriesDraft}
                                trafficMode={trafficModeDraft}
                                datesValid={datesValid}
                                dirty={hasUnsavedSettings}
                                loadSettings={activeLoadSettings}
                                onNameChange={setDraftName}
                                onStartDateChange={changeDraftStartDate}
                                onEndDateChange={setDraftEndDate}
                                onStayInCountryChange={setStayInCountryDraft}
                                onAvoidTollsChange={setAvoidTollsDraft}
                                onAvoidHighwaysChange={setAvoidHighwaysDraft}
                                onAvoidFerriesChange={setAvoidFerriesDraft}
                                onTrafficModeChange={setTrafficModeDraft}
                                onLoadSettingsChange={setLoadSettingsDraft}
                                ...
                                onDelete={() =>
                                  void confirm({
                                    title: "Placer cette sortie dans la corbeille ?",
                                    message: `La sortie « ${trip.name} » et toute sa planification pourront être restaurées pendant votre délai de conservation.`,
                                  }).then((confirmed) => {
                                    if (confirmed)
                                      void run(async () => {
                                        await deleteTrip(trip.id);
                                        await reload("");
                                        publishGlobalFeedback("success", `Sortie « ${trip.name} » déplacée dans la corbeille.`);
                                      });
                                  })
                                }
                              />
                            ...
                            <section className="trip-panel-section trip-panel-journeys">
                                  ...
                                  {canEditTrip && (
                                    <span className="trip-panel-journeys-route-actions">
                                      <button className={routeFeedback === "all" ? "route-success" : undefined} type="button" aria-label={pendingAction === "route-all" ? "Calcul des itinéraires en cours" : routeFeedback === "all" ? "Itinéraires rafraîchis" : "Calculer les itinéraires"} title={pendingAction === "route-all" ? "Calcul des itinéraires en cours" : routeFeedback === "all" ? "Itinéraires rafraîchis" : "Calculer les itinéraires"} disabled={busy || !trip.days.some((day, dayIndex) => canCalculateRoute(trip, day, dayIndex))} onClick={recalculateAllRoutes}>
                                        ...
                                        <span>{pendingAction === "route-all" ? "Calcul en cours…" : routeFeedback === "all" ? "Itinéraires rafraîchis" : "Calculer les itinéraires"}</span>
                                      </button>
                                      <button className="trip-global-optimize-button" type="button" aria-label={pendingAction === "optimize-all" ? "Optimisation du voyage en cours" : "Optimiser le voyage"} title={pendingAction === "optimize-all" ? "Optimisation du voyage en cours" : "Optimiser le voyage"} disabled={busy || globalOptimization !== null || !trip.days.some((day) => day.stops.length >= 2)} onClick={optimizeAllDays}>
                                        {pendingAction === "optimize-all" ? <LoaderCircle className="trip-action-spinner" size={13} aria-hidden="true" /> : <Sparkles size={13} />}
                                        <span>{pendingAction === "optimize-all" ? "Optimisation…" : "Optimiser le voyage"}</span>
                                      </button>
                                    </span>
                                  )}
                                </span>
                              ...
                              <TripAnchorActionsContext.Provider
                                value={{
                                  canEdit: canEditTrip,
                                  reload,
                                  onOpenPopup: (target) => onAnchorPopupChange(target),
                                  onPlaceDrop: onAnchorPlaceDrop,
                                }}
                              >
                                  <div className="trip-panel-days">
                                    {trip.days[0] && (
                                      <Departure
                                        trip={trip}
                                        selected={activeAnchorTarget === "departure"}
                                        recommendedStart={daySummaries[trip.days[0].id]?.recommended_start_time ?? null}
                                         recommendedStartOffset={daySummaries[trip.days[0].id]?.recommended_start_day_offset ?? null}
                                         collapseRequest={timelineCollapseRequest}
                                         onSelect={() => {
                                           expandTimelineItem("departure");
                                           setActiveNightTarget(null);
                                          onActiveNightTargetChange(null);
                                          onActiveAnchorTargetChange("departure");
                                          onActiveDayChange(trip.days[0].id);
                                        ...
                                        onStopPlaceSelect={onStopPlaceSelect}
                                      />
                                    )}
                                    {trip.days.map((day, dayIndex) => (
                                      <div
                                        key={day.id}
                                        className={`trip-timeline-day-block${draggedDayId === day.id ? " is-dragging" : ""}${dayDropTarget?.dayId === day.id && dayDropTarget.index === dayIndex ? " drop-before" : ""}${dayDropTarget?.dayId === day.id && dayDropTarget.index === dayIndex + 1 ? " drop-after" : ""}`}
                                        style={
                                          {
                                            "--trip-day-color": day.color ?? "#0FA68A",
                                          } as CSSProperties
                                        }
                                        onDragLeave={(event) => {
                                          if (draggedDayId && !event.currentTarget.contains(event.relatedTarget as Node | null)) setDayDropTarget(null);
                                        }}
                                        onDragOver={(event) => {
                                      ...
                                      >
                                        <DayVisibilityBubble day={day} hidden={hiddenDayIds.has(day.id)} onChange={(visible) => onDayVisibilityChange(day.id, visible)} />
                                          ...
                                          onClick={(event) => {
                                            const target = event.target as HTMLElement;
                                             ...
                                             activateDay(day.id);
                                             expandTimelineItem(`day:${day.id}`, day.id);
                                             if (target.closest(".trip-panel-day > summary")) event.preventDefault();
                                          }}
                                            onDragLeave={(event) => {
                                              if (!event.currentTarget.contains(event.relatedTarget as Node | null)) {
                                                setDropTarget((current) => (current?.dayId === day.id ? null : current));
                                                setPlaceDropDayId((current) => current === day.id ? null : current);
                                              }
                                            }}
                                            onDragOver={(event) => {
                                              event.preventDefault();
                                              if (canEditTrip && day.stops.length === 0 && hasDraggedPlace(event.dataTransfer)) {
                                                setPlaceDropDayId(day.id);
                                                setCollapsedDayIds((current) => {
                                                  if (!current.has(day.id)) return current;
                                                  const next = new Set(current);
                                                  next.delete(day.id);
                                                  return next;
                                                });
                                        ...
                                        >
                                          ...
                                          >
                                            <span className="trip-panel-day-heading">
                                              <span className="trip-panel-timeline-heading">
                                           ...
                                           <div className="trip-panel-day-content">
                                            {day.route_status === "stale" && <p>Itinéraire à recalculer</p>}
                                            {daySummaries[day.id]?.country_constraint_status === "unchecked" && <p className="trip-metrics-warning">Itinéraire à vérifier avec la contrainte pays.</p>}
                                            {daySummaries[day.id]?.country_constraint_status === "invalid" && <p className="trip-panel-error">Itinéraire refusé : passage hors de {daySummaries[day.id]?.constraint_country_name}.</p>}
                                            <ul>
                                              {day.stops.map((stop, index) => {
                                                const dropBefore = dropTarget?.dayId === day.id && dropTarget.index === index;
                                                const dropAfter = index === day.stops.length - 1 && dropTarget?.dayId === day.id && dropTarget.index === day.stops.length;
                                                return <li
                                                  key={stop.id}
                                                  className={`${draggedStopId === stop.id ? "is-dragging" : ""}${dropBefore ? " has-place-drop-before" : ""}${dropAfter ? " has-place-drop-after" : ""}`}
                                                  draggable={canEdit}
                                                  onDragStart={(event) => {
                                                    event.dataTransfer.effectAllowed = "move";
                                                    event.dataTransfer.setData("text/plain", `stop:${stop.id}`);
                                                  ...
                                                  onDragOver={(event) => {
                                                    if (!canEdit || draggedDayId) return;
                                                    event.preventDefault();
                                                    event.stopPropagation();
                                                    const bounds = event.currentTarget.getBoundingClientRect();
                                                    setDropTarget({
                                                      dayId: day.id,
                                                      index: index + Number(event.clientY > bounds.top + bounds.height / 2),
                                                    });
                                                  }}
                                                  onDrop={(event) => {
                                                    if (!draggedDayId) dropStop(event, day, dropTarget?.dayId === day.id ? dropTarget.index : index);
                                                  }}
                                                >
                                                  {dropBefore && <PlaceDropGhost moving={draggedStopId !== null} />}
                                                  <GripVertical className="trip-stop-grip" size={13} />
                                                  <i>{index + 1}</i>
                                                  ...
                                                  <span
                                                    ...
                                                    onClick={(event) => {
                                                      ...
                                                      activateDay(day.id);
                                                      onStopFocus?.(stop.latitude, stop.longitude);
                                                      if (stop.place_id) onStopPlaceSelect(stop.place_id);
                                                    }}
                                                    onKeyDown={(event) => {
                                                  ...
                                                  >
                                                    ...
                                                    {stop.stop_type !== "place" && <small>{stopTypeLabel(stop.stop_type)}</small>}
                                                  </span>
                                                  {!canEdit && (
                                                    <span className="trip-stop-duration">
                                                      <Clock3 aria-hidden="true" size={12} />
                                                      {formatMinutes(stop.visit_duration_minutes)}
                                                    </span>
                                                  )}
                                                  <span className="trip-stop-drive">
                                                    <Car aria-hidden="true" size={12} />
                                                    {formatRouteDuration(day.route_segments?.[index]?.duration_seconds ?? null)}
                                                  </span>
                                                  {canEdit && (
                                                    <VisitDurationControl
                                                      stop={stop}
                                                      disabled={busy}
                                                      onChange={async (minutes) => {
                                                        await updateTripStop(stop.id, {
                                                  ...
                                                  {stop.is_locked && <Lock size={11} />}
                                                  {canEdit && (
                                                    <span className="trip-panel-stop-actions">
                                                      <button
                                                        type="button"
                                                        ...
                                                        onClick={(event) => {
                                                          ...
                                                          void confirm({
                                                            title: "Supprimer cette étape ?",
                                                            message: `« ${stop.name} » sera retirée de la journée.`,
                                                          }).then((confirmed) => {
                                                            if (confirmed)
                                                              void run(() =>
                                                                runUndoable("suppression de l’étape", async () => {
                                                  ...
                                                  {dropAfter && <PlaceDropGhost moving={draggedStopId !== null} />}
                                                </li>;
                                              })}
                                            </ul>
                                            ...
                                            <div className={`trip-panel-route-actions${canEditTrip ? " trip-panel-route-actions--editable" : ""}`}>
                                              <button className={routeFeedback === day.id ? "route-success" : undefined} type="button" aria-label={pendingAction === `route:${day.id}` ? "Calcul de l’itinéraire en cours" : routeFeedback === day.id ? "Itinéraire rafraîchi" : "Itinéraire"} disabled={busy || !canEditTrip || !canCalculateRoute(trip, day, dayIndex)} onClick={() => recalculateRoute(day)}>
                                                ...
                                                ) : routeFeedback === day.id ? (
                                                  <>
                                                    <Check size={13} />
                                                    Itinéraire rafraîchi
                                                  </>
                                                ) : (
                                                  <>
                                                    <Route size={13} />
                                              ...
                                              <button
                                                type="button"
                                                aria-label={pendingAction === `optimize:${day.id}` ? "Optimisation de la journée en cours" : "Optimiser"}
                                                disabled={busy || day.stops.length < 2}
                                                onClick={() =>
                                                  void run(
                                                    async () =>
                                                      setOptimization({
                                              ...
                                              {canEdit && (
                                                <>
                                                  <button
                                                    type="button"
                                                    aria-label="Dupliquer la journée"
                                                    onClick={() =>
                                                      void run(() =>
                                                        runUndoable("duplication de la journée", async () => {
                                                          await duplicateTripDay(day.id);
                                                          await reload(trip.id);
                                                        }),
                                                      )
                                                    }
                                                  ...
                                                  <button
                                                    ...
                                                    onClick={() =>
                                                      void confirm({
                                                        ...
                                                        if (confirmed)
                                                          void run(() =>
                                                            runUndoable("suppression de la journée", async () => {
                                                              await deleteTripDay(day.id);
                                                              await reload(trip.id);
                                                            }),
                                                          );
                                              ...
                                              <button className="trip-day-settings-trigger" type="button" aria-expanded={openDaySettingsIds.has(day.id)} aria-controls={`trip-day-settings-${day.id}`} title="Réglages du jour" onClick={() => toggleDaySettings(day.id)}>
                                                <Settings2 aria-hidden="true" size={13} />
                                                Réglages
                                              </button>
                                            </div>
                                            {optimization?.dayId === day.id && (
                                              <div className="trip-panel-optimization">
                                                <OptimizationMetrics value={optimization.value} />
                                                <button type="button" onClick={() => setOptimization(null)}>
                                                  Refuser
                                                </button>
                                                <button
                                                  type="button"
                                                  onClick={() =>
                                                    void run(() =>
                                                      runUndoable("optimisation de la journée", async () => {
                                                        if (!optimization.value.proposal_id) throw new Error("La proposition d’optimisation est invalide.");
                                                        await confirmTripOptimization(day.id, optimization.value.proposal_id);
                                            ...
                                            <DaySettings
                                              ...
                                              onTimingSave={async (payload) => {
                                                await updateTripDayTiming(day.id, payload);
                                                await reload(trip.id);
                                              }}
                                              onColorSave={(color) =>
                                                void run(async () => {
                                                  await updateTripDay(day.id, {
                                                    color,
                                                  });
                                                  await reload(trip.id);
                                                })
                                        ...
                                        {dayIndex < trip.days.length - 1 && (
                                          <Night
                                            previous={day}
                                            next={trip.days[dayIndex + 1]}
                                            recommendedStart={daySummaries[trip.days[dayIndex + 1].id]?.recommended_start_time ?? null}
                                            recommendedStartOffset={daySummaries[trip.days[dayIndex + 1].id]?.recommended_start_day_offset ?? null}
                                            trip={trip}
                                            activeTarget={activeNightTarget}
                                            canEdit={canEditTrip}
                                            reload={reload}
                                            collapseRequest={timelineCollapseRequest}
                                             onSelect={(target, openPopup) => {
                                               expandTimelineItem(`night:${day.id}:${trip.days[dayIndex + 1].id}`);
                                               setActiveNightTarget(target);
                                              onActiveAnchorTargetChange(null);
                                              onActiveNightTargetChange(target, openPopup);
                                              onActiveDayChange(trip.days[dayIndex + 1].id);
                                            }}
                                            onStopFocus={onStopFocus}
                                          />
                                        )}
                                    ...
                                    {trip.days.length > 0 && (
                                      <Arrival
                                        trip={trip}
                                        selected={activeAnchorTarget === "arrival"}
                                        estimatedArrival={daySummaries[trip.days.at(-1)!.id]?.estimated_arrival_time ?? null}
                                        estimatedArrivalOffset={daySummaries[trip.days.at(-1)!.id]?.estimated_arrival_day_offset ?? null}
                                         collapseRequest={timelineCollapseRequest}
                                         onSelect={() => {
                                           expandTimelineItem("arrival");
                                           setActiveNightTarget(null);
                                          onActiveNightTargetChange(null);
                                          onActiveAnchorTargetChange("arrival");
                                          onActiveDayChange(trip.days.at(-1)!.id);
                                        }}
                                        onStopFocus={onStopFocus}
                                        onStopPlaceSelect={onStopPlaceSelect}
                                      />
              ...
              {isMobile && !tripViewOnly && trip && mobileDay && mobileTimelineTarget?.kind !== "night" && (
                <MobileTripDayNavigator
                  day={mobileDay}
                  days={trip.days}
                  canNavigate
                  onSwipeProgress={setMobileDaySwipeOffset}
                  onPrevious={() => {
                    const previousDay = trip.days[trip.days.findIndex((item) => item.id === mobileDay.id) - 1];
                    if (previousDay) activateMobileNight(previousDay.id, mobileDay.id, "backward");
                  }}
                  onNext={() => {
                    const nextDay = trip.days[trip.days.findIndex((item) => item.id === mobileDay.id) + 1];
                    if (nextDay) activateMobileNight(mobileDay.id, nextDay.id, "forward");
                  }}
                />
              )}
              ...
              {createOpen && (
                <CreateTripDialog
                  ...
                  onCreate={async (_mapId, payload) => {
                    const created = await createTrip(poiMap.id, payload);
                    await reload(created.id);
                    setCreateOpen(false);
                    publishGlobalFeedback("success", `Sortie « ${created.name} » créée.`);
                  }}
                />
              )}
              {mobileTripPickerOpen && (
                <MobileTripPickerDialog
                  trips={trips}
                  activeTripId={trip?.id ?? null}
                  busy={busy}
                  onClose={() => setMobileTripPickerOpen(false)}
                  onSelect={(id) => {
                    setMobileTripPickerOpen(false);
                    changeSelectedTrip(id);
                  }}
                />
              )}
              ...
              {isMobile && optimization && trip && (
                <MobileDayOptimizationDialog
                  ...
                  busy={busy}
                  onCancel={() => setOptimization(null)}
                  onApply={() =>
                    void run(() =>
                      runUndoable("optimisation de la journée", async () => {
                        if (!optimization.value.proposal_id) throw new Error("La proposition d’optimisation est invalide.");
                        await confirmTripOptimization(optimization.dayId, optimization.value.proposal_id);
                        setOptimization(null);
                        await reload(trip.id);
                      }),
                    )
...
function MobileTripSwipeSurface({ className, style, children, onPrevious, onNext, onSwipeProgress, disabled }: { className: string; style?: CSSProperties; children: ReactNode; onPrevious?: () => void; onNext?: () => void; onSwipeProgress?: (offset: number) => void; disabled?: boolean }) {
  ...
  return (
    <div className={`${className} trip-mobile-card-swipe`} style={style} {...swipeHandlers}>
      {children}
    </div>
  );
}
...
function MobileTripDayNavigator({ day, days, canNavigate, onSwipeProgress, onPrevious, onNext }: { day: TripDay; days: TripDay[]; canNavigate: boolean; onSwipeProgress: (offset: number) => void; onPrevious: () => void; onNext: () => void }) {
  ...
  const activateRelativeDay = (offset: number) => {
    if (offset < 0) {
      if (dayIndex > 0) onPrevious();
      else boundaries?.previous();
    }
    if (offset > 0) {
      if (dayIndex < days.length - 1) onNext();
      else boundaries?.next();
    }
  };
  ...
  return (
    <div
      className="trip-mobile-active-day"
      data-mobile-navigation-surface
      aria-label="Journée active"
    ...
    >
      <button className="trip-mobile-navigator__previous" type="button" disabled={!canNavigate || (dayIndex <= 0 && !boundaries)} onClick={() => activateRelativeDay(-1)}>
        <ChevronLeft size={15} aria-hidden="true" />
        <span>{previousLabel}</span>
      </button>
      <span className="trip-mobile-navigator__hint">
        <ChevronsLeft size={13} aria-hidden="true" />
        <small>Glisser pour naviguer</small>
        <ChevronsRight size={13} aria-hidden="true" />
      </span>
      <button className="trip-mobile-navigator__next" type="button" disabled={!canNavigate || (dayIndex >= days.length - 1 && !boundaries)} onClick={() => activateRelativeDay(1)}>
        <span>{nextLabel}</span>
...
function MobileTripOverview({ poiMap, trip, summary, busy, pendingAction, onRoute, onOptimize, onTimeline, onOffline, onSettings, onExport }: { poiMap: PoiMap; trip: Trip; summary: TripSummary | null; busy: boolean; pendingAction: TripActionKey | null; onRoute: () => void; onOptimize: () => void; onTimeline: () => void; onOffline: () => void; onSettings: () => void; onExport: () => void }) {
  ...
  return (
    <section className="trip-mobile-overview" aria-label="Résumé de la sortie">
      <header className="trip-mobile-overview__header">
        <div className="trip-mobile-overview__title-row">
          <h2>Sortie</h2>
          <strong title={trip.name}>{trip.name}</strong>
          <em className={`trip-mobile-overview__status trip-mobile-overview__status--${trip.status}`}>{statusLabel}</em>
        </div>
        <div className="trip-mobile-overview__meta">
          <span className="trip-mobile-overview__country">
            <CountryFlag countryCode={poiMap.country?.iso_alpha2 ?? ""} className="trip-mobile-overview__flag" fallbackSize={14} />
            {countryName}
          </span>
          <span aria-hidden="true">·</span>
          <span>
            {trip.days.length} {trip.days.length > 1 ? "jours" : "jour"}
          </span>
          <span aria-hidden="true">·</span>
          <span title="Distance totale">
            <Road aria-hidden="true" size={13} />
            {formatRouteDistance(summary?.total_route_distance_meters ?? null)}
          </span>
          <span title="Temps total">
            <Clock3 aria-hidden="true" size={13} />
            {formatMinutes(summary?.total_planned_duration_minutes ?? null)}
          </span>
        </div>
      </header>
      <div className="trip-mobile-overview__actions">
        <button
          type="button"
          className="trip-mobile-overview__create"
          aria-label="Ajouter une sortie"
          title="Ajouter une sortie"
          onClick={() =>
            window.dispatchEvent(
              new CustomEvent("cartavault:mobile-day-action", {
                detail: { action: "create-trip" },
              }),
            )
          }
        >
        ...
        <button className="trip-mobile-overview__open" type="button" aria-label="Ouvrir une sortie" title="Ouvrir une sortie" onClick={() => mobileActions?.openPicker()}>
          <FolderOpen size={18} />
        </button>
        <button type="button" disabled={busy} aria-busy={pendingAction === "route-all"} onClick={onRoute}>
          {pendingAction === "route-all" ? <LoaderCircle className="trip-action-spinner" size={17} aria-hidden="true" /> : <Route size={17} aria-hidden="true" />}
          <span>Itinéraire global</span>
        </button>
        <button type="button" disabled={busy} aria-busy={pendingAction === "optimize-all"} onClick={onOptimize}>
          {pendingAction === "optimize-all" ? <LoaderCircle className="trip-action-spinner" size={17} aria-hidden="true" /> : <Sparkles size={17} aria-hidden="true" />}
          <span>Optimiser</span>
        </button>
        <span>
          <button type="button" aria-label="Plus d’actions pour la sortie" aria-expanded={moreOpen} onClick={() => setMoreOpen((current) => !current)}>
            <Ellipsis size={19} />
          </button>
          {moreOpen && (
            <div role="menu">
              <button type="button" role="menuitem" onClick={() => closeThen(onTimeline)}>
                <IconTimelineEvent size={16} />
                Mode chronologie
              </button>
              <button type="button" role="menuitem" onClick={() => closeThen(onOffline)}>
                <HardDriveDownload size={16} />
                Mettre hors-ligne
              </button>
              <button type="button" role="menuitem" onClick={() => closeThen(onSettings)}>
                <SlidersHorizontal size={16} />
                Paramètres de la sortie
              </button>
              <button type="button" role="menuitem" onClick={() => closeThen(onExport)}>
                <Download size={16} />
                Exporter la sortie
              </button>
            </div>
...
function MobileTripPickerDialog({ trips, activeTripId, busy, onClose, onSelect }: { trips: Trip[]; activeTripId: string | null; busy: boolean; onClose: () => void; onSelect: (id: string) => void }) {
  return createPortal(
    <div
      className="cv-overlay trip-mobile-picker-overlay"
      role="presentation"
      onMouseDown={(event) => {
    ...
    >
      <section className="trip-mobile-picker-dialog cv-modal" role="dialog" aria-modal="true" aria-labelledby="trip-mobile-picker-title">
        <header>
          <div>
            <p className="cv-workspace-panel__eyebrow">Sorties de la carte</p>
            <h2 id="trip-mobile-picker-title">Ouvrir une sortie</h2>
          </div>
          <button className="panel-icon-button" type="button" aria-label="Fermer" disabled={busy} onClick={onClose}>
            <X size={17} />
          </button>
        </header>
        <div>
          {trips.length === 0 ? (
            ...
            trips.map((item) => (
              <button key={item.id} type="button" className={item.id === activeTripId ? "is-active" : undefined} disabled={busy} onClick={() => onSelect(item.id)}>
                <Map size={17} />
                <span>
                  <strong>{item.name}</strong>
                  <small>
                    {item.days.length} {item.days.length > 1 ? "jours" : "jour"}
                  </small>
                </span>
...
function MobileDayOptimizationDialog({ day, value, busy, onCancel, onApply }: { day: TripDay | null; value: TripOptimization; busy: boolean; onCancel: () => void; onApply: () => void }) {
  return createPortal(
    <div className="cv-overlay trip-mobile-optimization-overlay trip-mobile-day-optimization-overlay" role="presentation">
      <section className="trip-mobile-optimization-dialog cv-modal" role="dialog" aria-modal="true" aria-labelledby="trip-mobile-day-optimization-title">
        <header>
          <div>
            <p className="cv-workspace-panel__eyebrow">Optimisation prête</p>
            <h2 id="trip-mobile-day-optimization-title">Résultat du jour {day?.day_number ?? ""}</h2>
          </div>
        ...
        <footer>
          ...
          <button className="primary" type="button" disabled={busy} onClick={onApply}>
            <Check size={15} />
            Accepter
          </button>
        </footer>
...
function MobileGlobalOptimizationDialog({ proposals, busy, onCancel, onApply }: { proposals: TripOptimization[]; busy: boolean; onCancel: () => void; onApply: () => void }) {
  return createPortal(
    <div className="cv-overlay trip-mobile-optimization-overlay" role="presentation">
      <GlobalOptimizationReview proposals={proposals} busy={busy} onCancel={onCancel} onApply={onApply} />
    </div>,
    document.body,
  );
}
...
function MobileTripNightFooterNavigator({ previousDay, nextDay, onPrevious, onNext }: { previousDay: TripDay; nextDay: TripDay; onPrevious: () => void; onNext: () => void }) {
  ...
  return (
    <div
      className="trip-mobile-active-day"
      aria-label={`Navigation de la nuit ${previousDay.day_number}`}
      onPointerDown={(event) => {
    ...
    >
      <button className="trip-mobile-navigator__previous" type="button" aria-label={`Revenir au jour ${previousDay.day_number}`} onClick={onPrevious}>
        <ChevronLeft size={15} aria-hidden="true" />
        <span>Jour {previousDay.day_number}</span>
      </button>
      <span className="trip-mobile-navigator__hint">
        <ChevronsLeft size={13} aria-hidden="true" />
        <small>Glisser pour naviguer</small>
      ...
      <button className="trip-mobile-navigator__next" type="button" aria-label={`Aller au jour ${nextDay.day_number}`} onClick={onNext}>
        <span>Jour {nextDay.day_number}</span>
        <ChevronRight size={15} aria-hidden="true" />
      </button>
    </div>
  );
...
function MobileTripAnchorPage({ target, anchor, onOpen, onPrevious, onNext, onSwipeProgress, swipeOffset }: { target: "departure" | "arrival"; anchor: NonNullable<Trip["departure"]> | NonNullable<Trip["arrival"]> | null; onOpen: () => void; onPrevious?: () => void; onNext?: () => void; onSwipeProgress: (offset: number) => void; swipeOffset: number }) {
  ...
  return (
    <section className={`trip-mobile-anchor-page trip-mobile-anchor-page--${target} trip-mobile-step-commands trip-mobile-card-swipe${swipeOffset ? " is-swiping" : ""}`} style={{ "--trip-mobile-swipe-offset": `${swipeOffset}px` } as CSSProperties} aria-label={label} {...swipeHandlers}>
      <header>
        <strong>{label}</strong>
        <span>1 étape</span>
      </header>
      <ul>
        <li>
          <button className="trip-mobile-step-row" type="button" disabled={!anchor} onClick={onOpen}>
            <span>
              <b>1</b>
              <MapPin className="trip-mobile-step-pin" aria-hidden="true" size={15} />
              <strong>{anchor?.name ?? `${label} non défini`}</strong>
            </span>
          </button>
        </li>
      </ul>
...
function MobileTripAnchorFooter({ target, previousLabel, nextLabel, disabledPrevious = false, disabledNext = false, onPrevious, onNext }: { target: "departure" | "arrival"; previousLabel: string; nextLabel: string; disabledPrevious?: boolean; disabledNext?: boolean; onPrevious: () => void; onNext: () => void }) {
  ...
  const finishSwipe = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const deltaY = event.clientY - start.y;
    if (Math.abs(deltaX) < 44 || Math.abs(deltaX) <= Math.abs(deltaY) * 1.2) return;
    if (deltaX < 0 && !disabledNext) onNext();
    if (deltaX > 0 && !disabledPrevious) onPrevious();
  };
  const label = target === "departure" ? "Départ" : "Arrivée";
  return (
    <div
    ...
    >
      <button className="trip-mobile-navigator__previous" type="button" disabled={disabledPrevious} onClick={onPrevious}>
        <ChevronLeft size={15} aria-hidden="true" />
        <span>{previousLabel}</span>
      </button>
      <span className="trip-mobile-navigator__hint">
      ...
      <button className="trip-mobile-navigator__next" type="button" disabled={disabledNext} onClick={onNext}>
        <span>{nextLabel}</span>
        <ChevronRight size={15} aria-hidden="true" />
      </button>
    </div>
...
function MobileTripNightNavigator({ previousDay, nextDay, night, days, nights, canEdit, busy, onPrevious, onNext, onOpenPopup, onDelete, onMoveToDay, onMoveToNight }: { previousDay: TripDay; nextDay: TripDay; night: Trip["nights"][number] | null; days: TripDay[]; nights: MobileNightDestination[]; canEdit: boolean; busy: boolean; onPrevious: () => void; onNext: () => void; onOpenPopup: () => void; onDelete: (night: Trip["nights"][number]) => void; onMoveToDay: (night: Trip["nights"][number], dayId: string) => void; onMoveToNight: (night: Trip["nights"][number], destination: MobileNightDestination) => void }) {
  ...
  const startPointSwipe = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    event.stopPropagation();
    event.currentTarget.setPointerCapture?.(event.pointerId);
    pointSwipeRef.current = {
      pointerId: event.pointerId,
      x: event.clientX,
      y: event.clientY,
      baseOffset: pointSwipeOffsetRef.current,
      axis: "pending",
    };
  };
  ...
  return (
    <>
      <section
        className="trip-mobile-night-page trip-mobile-step-commands"
        aria-label={`Nuit ${previousDay.day_number}`}
        style={
          {
            "--trip-mobile-day-color": nextDay.color ?? "#0FA68A",
            "--trip-mobile-night-from": previousDay.color ?? "#0FA68A",
            "--trip-mobile-night-to": nextDay.color ?? "#0FA68A",
          } as CSSProperties
        }
      >
        <header className="trip-mobile-active-day">
          <button type="button" aria-label={`Revenir au jour ${previousDay.day_number}`} onClick={onPrevious}>
            <ChevronLeft size={16} aria-hidden="true" />
          </button>
          <strong>Nuit {previousDay.day_number}</strong>
          <span>Transition</span>
          <button type="button" aria-label={`Aller au jour ${nextDay.day_number}`} onClick={onNext}>
            <ChevronRight size={16} aria-hidden="true" />
          </button>
        </header>
        <header className="trip-mobile-stage-heading">
          <strong>Nuit {previousDay.day_number}</strong>
          <span>1 étape</span>
        </header>
        <ul>
          <li ref={pointSwipeContainerRef} className={`trip-mobile-night-swipe${pointSwipeOffset >= 22 ? " is-delete-revealed" : pointSwipeOffset <= -34 ? " is-more-revealed" : ""}`}>
            {night && canEdit && (
              <button className="trip-mobile-swipe-delete" type="button" aria-label={`Supprimer ${night.name}`} disabled={busy} onClick={() => onDelete(night)}>
                <Trash2 size={17} />
              </button>
            )}
            {night && (
              <div className="trip-mobile-swipe-more" onPointerDown={(event) => event.stopPropagation()}>
                <button type="button" aria-label={`Déplacer ${night.name}`} title="Envoyer vers" disabled={busy || !canEdit} onClick={() => setMoveTargetOpen(true)}>
                  <CalendarDays size={17} />
                </button>
                <a className="trip-mobile-swipe-map" href={googleMapsPointUrl(night)} target="_blank" rel="noopener noreferrer" aria-label={`Ouvrir ${night.name} dans Google Maps`} onClick={(event) => event.stopPropagation()}>
                  <GoogleMapsIcon size={24} />
                </a>
              </div>
            )}
            <div
              className={`trip-mobile-night-page__card trip-mobile-step-row${night ? " is-interactive" : ""}`}
              role={night ? "button" : undefined}
              tabIndex={night ? 0 : undefined}
              aria-label={night ? `Ouvrir ${night.name}` : undefined}
              onPointerDown={startPointSwipe}
              onPointerMove={movePointSwipe}
              onPointerUp={finishPointSwipe}
              onPointerCancel={() => {
                pointSwipeRef.current = null;
                setNightOffset(0);
              }}
              onKeyDown={(event) => {
                if (night && (event.key === "Enter" || event.key === " ")) {
                  event.preventDefault();
                  onOpenPopup();
                }
              }}
              style={
                {
                  "--trip-mobile-night-from": previousDay.color ?? "#0FA68A",
                  "--trip-mobile-night-to": nextDay.color ?? "#0FA68A",
                  transform: `translateX(${pointSwipeOffset}px)`,
                } as CSSProperties
              }
            >
              <span>
                <b>1</b>
                <MapPin className="trip-mobile-step-pin" aria-hidden="true" size={15} />
                <strong>{night?.name ?? "Nuit non renseignée"}</strong>
              </span>
            </div>
          </li>
        </ul>
      </section>
      {night && moveTargetOpen && (
        <MobileMoveNightDialog
          night={night}
          days={days}
          nights={nights}
          busy={busy}
          ...
          onMoveToDay={(dayId) => {
            ...
            onMoveToDay(night, dayId);
          }}
          onMoveToNight={(destination) => {
            setMoveTargetOpen(false);
            onMoveToNight(night, destination);
          }}
        />
      )}
    </>
...
function MobileTripDayMetrics({ summary }: { summary: TripDayTimeSummary | undefined }) {
  return (
    <section className="trip-mobile-day-metrics" aria-label="Résumé de la journée active">
      <span title="Distance de la journée">
        <Road aria-hidden="true" size={14} />
        <strong>{formatRouteDistance(summary?.route_distance_meters ?? null)}</strong>
      </span>
      <span title="Temps de trajet de la journée">
        <Navigation aria-hidden="true" size={14} />
        <strong>{formatMinutes(summary?.route_duration_minutes ?? null)}</strong>
      </span>
      <span title="Temps total de la journée">
        <Clock3 aria-hidden="true" size={14} />
        <strong>{formatMinutes(summary?.total_duration_minutes ?? null)}</strong>
      </span>
    </section>
  );
}
  ...
  return (
    <>
      <section className="trip-mobile-step-commands" style={{ "--trip-mobile-day-color": day.color ?? "#0FA68A" } as CSSProperties} aria-label={`Étapes de ${day.title || `Jour ${day.day_number}`}`}>
        <header>
          <strong>Jour {day.day_number}</strong>
          <span className="trip-mobile-day-header-metric" title="Distance">
            <Road size={13} aria-hidden="true" />
            {formatRouteDistance(day.route_distance_meters)}
          </span>
          <span className="trip-mobile-day-header-metric" title="Temps de trajet">
            <Navigation size={13} aria-hidden="true" />
            {formatRouteDuration(day.route_duration_seconds)}
          </span>
          <span className="trip-mobile-day-header-metric" title="Temps total">
            <Clock3 size={13} aria-hidden="true" />
            {formatMinutes(day.total_duration_minutes)}
          </span>
        </header>
        <div className="trip-mobile-day-actions" aria-label={`Actions du jour ${day.day_number}`}>
          <button
            type="button"
            disabled={busy}
            aria-busy={pendingAction === `route:${day.id}`}
            onClick={() =>
              onRoute?.() ??
              window.dispatchEvent(
                new CustomEvent("cartavault:mobile-day-action", {
                  detail: { action: "route", dayId: day.id },
                }),
              )
            }
          >
            {pendingAction === `route:${day.id}` ? <LoaderCircle className="trip-action-spinner" size={17} aria-hidden="true" /> : <Route size={17} aria-hidden="true" />}
            <span>Itinéraire du jour</span>
          </button>
          <button
            type="button"
            disabled={busy || day.stops.length < 2}
            aria-busy={pendingAction === `optimize:${day.id}`}
            onClick={() =>
              onOptimize?.() ??
              window.dispatchEvent(
                new CustomEvent("cartavault:mobile-day-action", {
                  detail: { action: "optimize", dayId: day.id },
                }),
              )
            }
          >
            {pendingAction === `optimize:${day.id}` ? <LoaderCircle className="trip-action-spinner" size={17} aria-hidden="true" /> : <Sparkles size={17} aria-hidden="true" />}
            <span>Optimiser le jour</span>
          </button>
          <button
            type="button"
            className="trip-mobile-day-actions__add"
            aria-label="Ajouter une journée"
            title="Ajouter une journée"
            disabled={busy}
            onClick={() =>
              window.dispatchEvent(
                new CustomEvent("cartavault:mobile-day-action", {
                  detail: { action: "add-day", dayId: day.id },
                }),
              )
            }
          >
          ...
          {dayActions && <MobileTripDayMoreMenu day={day} disabled={busy} actions={dayActions} variant="actions" />}
        </div>
        {day.stops.length === 0 ? (
          <p>Ajoutez une étape avec le bouton +.</p>
        ) : (
          <ul>
            {day.stops.map((stop, index) => (
              <li
                ...
                data-mobile-stop-index={index}
                className={`${draggedStopId === stop.id ? "is-touch-dragging" : ""}${touchDropIndex === index ? " drop-before" : ""}${touchDropIndex === index + 1 ? " drop-after" : ""}${deleteRevealedStopId === stop.id ? " is-delete-revealed" : ""}${moreRevealedStopId === stop.id ? " is-more-revealed" : ""}`}
                onPointerDown={(event) => beginSwipe(event, stop.id)}
                onPointerMove={moveSwipe}
                onPointerUp={finishSwipe}
                onPointerCancel={() => {
                  swipeStartRef.current = null;
              ...
              >
                <button className="trip-mobile-swipe-delete" type="button" aria-label={`Supprimer ${stop.name}`} title="Supprimer" disabled={busy} onClick={() => onDelete(stop.id, stop.name)}>
                  <Trash2 size={17} />
                </button>
                <div className="trip-mobile-swipe-more">
                  <button type="button" aria-label={`Déplacer ${stop.name} vers un autre jour`} title="Déplacer vers un autre jour" disabled={busy} onClick={() => setMoveTarget(stop)}>
                    <CalendarDays size={17} />
                  </button>
                  <button type="button" className="trip-mobile-swipe-map" aria-label={`Afficher ${stop.name} sur la carte`} title="Afficher sur la carte" onClick={() => onOpenStop(stop)}>
                    <Map size={17} />
                  </button>
                </div>
                <div
                  className="trip-mobile-step-row"
                  role="button"
                  tabIndex={0}
                  aria-label={`Ouvrir ${stop.name}`}
                  onClick={(event) => {
                  ...
                  style={{
                    transform: `translateX(${swipeOffset?.stopId === stop.id ? swipeOffset.offset : deleteRevealedStopId === stop.id ? 38 : moreRevealedStopId === stop.id ? -78 : 0}px)`,
                  }}
                >
                  <span>
                    <button className="trip-mobile-drag-handle" type="button" aria-label={`Maintenir pour déplacer ${stop.name}`} title="Maintenir pour déplacer" disabled={busy} onPointerDown={(event) => beginTouchReorder(event, stop.id)} onPointerMove={moveTouchReorder} onPointerUp={(event) => finishTouchReorder(event, stop.id, index)} onPointerCancel={cancelTouchReorder}>
        ...
        <button className="trip-mobile-add-place" type="button" aria-label={`Ajouter un lieu au ${day.title || `jour ${day.day_number}`}`} title="Ajouter un lieu" disabled={busy} onClick={onAddPlaces}>
      ...
      {moveTarget && (
        <MobileMoveDayDialog
          ...
          onSelect={(targetDay) => {
            onMove(moveTarget.id, targetDay.id, targetDay.stops.length);
            setMoveTarget(null);
          }}
        />
      )}
...
function MobileMoveDayDialog({ stop, days, busy, onClose, onSelect }: { stop: TripStop; days: TripDay[]; busy: boolean; onClose: () => void; onSelect: (day: TripDay) => void }) {
  return createPortal(
    <div
      className="cv-overlay trip-mobile-move-overlay"
      role="presentation"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget && !busy) onClose();
      }}
    >
      <section className="trip-mobile-move-dialog cv-modal" role="dialog" aria-modal="true" aria-labelledby="trip-mobile-move-title">
        <header>
          <div>
            <p className="cv-workspace-panel__eyebrow">Déplacer une étape</p>
            <h2 id="trip-mobile-move-title">Envoyer vers un jour</h2>
            <span>{stop.name}</span>
          </div>
          <button className="panel-icon-button" type="button" aria-label="Fermer" disabled={busy} onClick={onClose}>
            <X size={17} />
        ...
        <div className="trip-mobile-move-dialog__days">
          {days.map((day) => (
            <button key={day.id} type="button" disabled={busy} onClick={() => onSelect(day)}>
              <CalendarDays size={16} />
              <span>
                <strong>Jour {day.day_number}</strong>
                <small>{day.title || `${day.stops.length} étape${day.stops.length > 1 ? "s" : ""}`}</small>
              </span>
...
function MobileTripDayMoreMenu({ day, disabled, actions, variant = "header" }: { day: TripDay; disabled: boolean; actions: MobileDayActionsContextValue; variant?: "header" | "actions" }) {
  ...
  return (
    <span className={`trip-mobile-day-more trip-mobile-day-more--${variant}`}>
      <button type="button" aria-label={`Plus d’actions pour le jour ${day.day_number}`} aria-expanded={open} disabled={disabled} onClick={() => setOpen((current) => !current)}>
        <Ellipsis size={18} />
      </button>
      {open && (
        <div role="menu">
          <button type="button" role="menuitem" onClick={() => closeThen(() => actions.openSettings(day.id))}>
            <Settings2 size={15} />
            Réglages du jour
          </button>
          <button type="button" role="menuitem" onClick={() => closeThen(() => actions.remove(day))}>
            <Trash2 size={15} />
            Supprimer le jour
          </button>
          <button type="button" role="menuitem" onClick={() => closeThen(() => actions.duplicate(day.id))}>
            <Copy size={15} />
            Dupliquer le jour
          </button>
        </div>
  ...
  return createPortal(
    ...
    >
      <section className="trip-mobile-move-dialog cv-modal" role="dialog" aria-modal="true" aria-labelledby="trip-mobile-move-night-title">
        <header>
          <div>
            <p className="cv-workspace-panel__eyebrow">Déplacer une nuit</p>
            <h2 id="trip-mobile-move-night-title">Envoyer vers</h2>
        ...
        <div className="trip-mobile-move-dialog__days">
          {days.map((day) => (
            <button key={`day:${day.id}`} type="button" disabled={busy} onClick={() => onMoveToDay(day.id)}>
              <CalendarDays size={16} />
              <span>
                <strong>Jour {day.day_number}</strong>
                <small>{day.title || `${day.stops.length} étape${day.stops.length > 1 ? "s" : ""}`}</small>
          ...
          {nights
            .filter((item) => item.previousDayId !== night.previous_day_id)
            .map((destination) => (
              <button key={`night:${destination.previousDayId}`} type="button" disabled={busy} onClick={() => onMoveToNight(destination)}>
                <Moon size={16} />
                <span>
                  <strong>{destination.label}</strong>
                  <small>Entre deux journées</small>
                </span>
                <ChevronRight size={16} />
              </button>
            ))}
...
function TripSettings({ trip, canEdit, canEditStructure, canManage, canDelete, busy, draftName, draftStartDate, draftEndDate, stayInCountry, avoidTolls, avoidHighways, avoidFerries, trafficMode, datesValid, dirty, loadSettings, onNameChange, onStartDateChange, onEndDateChange, onStayInCountryChange, onAvoidTollsChange, onAvoidHighwaysChange, onAvoidFerriesChange, onTrafficModeChange, onLoadSettingsChange, onSave, onDuplicate, onArchive, onUnarchive, onDelete }: { trip: Trip; canEdit: boolean; canEditStructure: boolean; canManage: boolean; canDelete: boolean; busy: boolean; draftName: string; draftStartDate: string | null; draftEndDate: string | null; stayInCountry: boolean; avoidTolls: boolean; avoidHighways: boolean; avoidFerries: boolean; trafficMode: NonNullable<Trip["traffic_mode"]>; datesValid: boolean; dirty: boolean; loadSettings: TripLoadSettings; onNameChange: (value: string) => void; onStartDateChange: (value: string | null) => void; onEndDateChange: (value: string | null) => void; onStayInCountryChange: (value: boolean) => void; onAvoidTollsChange: (value: boolean) => void; onAvoidHighwaysChange: (value: boolean) => void; onAvoidFerriesChange: (value: boolean) => void; onTrafficModeChange: (value: NonNullable<Trip["traffic_mode"]>) => void; onLoadSettingsChange: (settings: TripLoadSettings) => void; onSave: () => void; onDuplicate: () => void; onArchive: () => void; onUnarchive: () => void; onDelete: () => void }) {
  ...
  return (
    <section className="trip-panel-section trip-panel-settings" aria-labelledby="trip-settings-title">
      <header className="trip-panel-settings__header">
        <span id="trip-settings-title">Paramètres de la sortie</span>
        <button
          className="trip-mobile-settings-close"
          type="button"
          aria-label="Fermer les paramètres"
          onClick={() =>
            window.dispatchEvent(
              new CustomEvent("cartavault:mobile-day-action", {
                detail: { action: "close-settings" },
              }),
            )
          }
        >
      ...
      <section className="trip-panel-options">
        <h3>Nom du voyage</h3>
        <div className="trip-panel-fields">
          <input aria-label="Nom du voyage" value={draftName} readOnly={!canEdit} onChange={(event) => onNameChange(event.target.value)} />
          <div className="trip-panel-field-meta">
            <span className={dirty ? "dirty" : ""}>{dirty ? "Non enregistré" : "Enregistré"}</span>
          </div>
        </div>
      </section>
      <section className="trip-settings-dates" aria-label="Dates du voyage">
        <h3>Dates du voyage</h3>
        <div>
          <div className="trip-settings-date-field">
        ...
        {!datesValid && (
          <p className="trip-settings-date-error" role="alert">
            La date d’arrivée doit être égale ou postérieure à la date de départ.
          </p>
        )}
      </section>
      <section className="trip-panel-options trip-country-setting">
        <h3>Itinéraire</h3>
        <label className="trip-country-setting__control">
          <span className="trip-country-setting__icon" aria-hidden="true">
            <Route size={16} />
          </span>
          <span className="trip-country-setting__copy">
      ...
      {canManage && (
        <section className="trip-settings-controls" aria-label="Contrôles de la sortie">
          ...
          <div>
            <button type="button" aria-label="Dupliquer le voyage" disabled={busy} onClick={onDuplicate}>
              <Copy size={14} />
              Dupliquer
            </button>
            {archived ? (
              <button className="trip-settings-control--reactivate" type="button" aria-label="Réactiver la sortie" disabled={busy} onClick={onUnarchive}>
                <ArchiveRestore size={14} />
                Réactiver
              </button>
            ) : (
              <button type="button" aria-label="Archiver la sortie" disabled={busy} onClick={onArchive}>
                <Archive size={14} />
                Archiver
              </button>
            )}
            {canDelete && (
              <button className="trip-settings-control--danger" type="button" aria-label="Supprimer le voyage" disabled={busy} onClick={onDelete}>
                <Trash2 size={14} />
                Supprimer
              </button>
            )}
            {canEdit && (
              <button className="trip-settings-control--save" type="button" aria-label="Enregistrer" disabled={busy || !dirty || !datesValid || loadSettings.low_load_max_minutes >= loadSettings.medium_load_max_minutes} onClick={onSave}>
                <Save size={14} />
                Enregistrer
              </button>
            )}
...
function sameLoadSettings(left: TripLoadSettings, right: TripLoadSettings) {
  return left.low_load_max_minutes === right.low_load_max_minutes && left.medium_load_max_minutes === right.medium_load_max_minutes && left.low_load_color === right.low_load_color && left.medium_load_color === right.medium_load_color && left.high_load_color === right.high_load_color;
}

function TripSummaryMetrics({ summary, preview = false }: { summary: TripSummary; preview?: boolean }) {
  return (
    <section className="trip-summary-shell">
      <div className={`trip-summary-primary${preview ? " trip-summary-primary--preview" : ""}`} aria-label="Chiffres clés du voyage">
        <div>
          <Road aria-hidden="true" size={24} />
          <span>
            <strong>{formatRouteDistance(summary.total_route_distance_meters)}</strong>
        ...
        <div>
          ...
          <span>
            <strong>{formatMinutes(summary.total_route_duration_minutes)}</strong>
            <small>Temps de trajet</small>
          </span>
        </div>
        <div>
        ...
        {preview && (
          <div>
            ...
            <span>
              <strong>{formatMinutes(summary.total_visit_duration_minutes)}</strong>
              <small>Temps de visite</small>
            </span>
          </div>
        )}
      </div>
      {!preview && (
        <details className="trip-metrics trip-metrics-global">
          <summary aria-label="Afficher plus d’informations sur le voyage">
            <small>Afficher plus d’infos</small>
            <ChevronDown className="trip-panel-chevron" size={15} />
          </summary>
          <div className="trip-metrics__body" aria-label="Informations détaillées du voyage">
            <div className="trip-metrics-group">
              <strong>Trajet total</strong>
              <dl>
                <Metric label="Distance totale de route" value={formatRouteDistance(summary.total_route_distance_meters)} />
                <Metric label="Temps total de conduite" value={formatMinutes(summary.total_route_duration_minutes)} />
              </dl>
            </div>
            <div className="trip-metrics-group">
              <strong>Durée planifiée</strong>
              <dl>
                <Metric label="Visites" value={formatMinutes(summary.total_visit_duration_minutes)} />
                <Metric label="Temps tampon" value={formatMinutes(summary.total_buffer_duration_minutes)} />
                <Metric label="Marges de sécurité" value={formatMinutes(summary.total_safety_margin_minutes)} />
                <Metric label="Durée totale estimée" value={formatMinutes(summary.total_planned_duration_minutes)} />
              </dl>
            </div>
            <div className="trip-metrics-group">
              <strong>Charge des journées</strong>
              <dl>
                <Metric label="Légères" value={String(summary.low_load_days)} />
                <Metric label="Moyennes" value={String(summary.medium_load_days)} />
                <Metric label="Élevées" value={String(summary.high_load_days)} />
              </dl>
            </div>
            {!summary.is_time_summary_complete && (
              <p className="trip-metrics-warning" role="status">
                Résumé partiel : {summary.days_with_incomplete_time_summary} {summary.days_with_incomplete_time_summary > 1 ? "journées sans planification complète" : "journée sans planification complète"}.
              </p>
            )}
          </div>
        </details>
...
function routeSegmentBetweenStops(trip: Trip, day: TripDay, dayIndex: number, stopIndex: number) {
  ...
  const hasStartAnchor = dayIndex === 0 ? trip.departure !== null : trip.nights.some((night) => night.next_day_id === day.id) || (trip.days[dayIndex - 1]?.stops.length ?? 0) > 0;
...
function TripPreviewTimeline({ trip, activeDayId, selectedKey, daySummaries, onSelectDay, onSelectNight, onSelectLocation, onNavigateItem }: TripPreviewTimelineProps) {
  ...
  const effectiveArrival = trip.arrival;
  ...
  const selectedLeg = useMemo(() => {
    ...
    if (selectedKey === "departure") {
      ...
      return {
        ...
        to: target.name,
        color: day.color ?? "#0FA68A",
        segment: routeSegmentFromDepartureToStop(day, trip.departure.id, target.id),
      };
    }
    if (selectedKey.startsWith("night:")) {
      ...
      return {
        ...
        to: target.name,
        color: day.color ?? "#0FA68A",
        segment: routeSegmentFromNightToStop(day, night.id, target.id),
      };
    }
    ...
    const segment = nextStop ? routeSegmentBetweenStops(trip, day, dayIndex, stopIndex) : nextNight ? routeSegmentFromStopToNight(day, stopIndex, nextNight.id) : nextArrival ? routeSegmentFromStopToArrival(day, stopIndex, nextArrival.id) : null;
    return {
      from: stop.name,
      to: target.name,
      color: day.color ?? "#0FA68A",
  ...
  const centerTimelineElement = useCallback(
    (item: HTMLElement, behavior: "smooth" | "instant" = "smooth") => {
  ...
  const lockTimelineElementToGuide = useCallback(
    (item: HTMLElement) => {
      ...
      const correctAlignment = () => {
        ...
        if (Math.abs(error) <= 0.25 || remainingCorrections <= 0) {
          alignmentFrameRef.current = null;
          return;
        }
  ...
  const centerSelectedTimelineElement = useCallback(
  ...
  const findTimelineItemNearestCenter = useCallback(() => {
    ...
    const candidates = Array.from(viewport.querySelectorAll<HTMLElement>("[data-preview-navigation-index]"))
      .map((element) => ({
        element,
        index: Number(element.dataset.previewNavigationIndex),
        bounds: getTimelinePointBounds(element),
      }))
  ...
  const navigateTimeline = useCallback(
    (direction: -1 | 1) => {
      ...
      const nextIndex = selectedNavigationIndex < 0 ? (direction === 1 ? 0 : navigationItems.length - 1) : Math.min(navigationItems.length - 1, Math.max(0, selectedNavigationIndex + direction));
  ...
  const navigateTimelineWithWheel = useCallback(
    (event: WheelEvent) => {
  ...
  useEffect(() => {
    const viewport = trackRef.current;
    if (!viewport) return;
    viewport.addEventListener("wheel", navigateTimelineWithWheel, {
      passive: false,
    });
  ...
  return (
    <section className="trip-preview-timeline" aria-labelledby="trip-preview-timeline-title">
      <h3 id="trip-preview-timeline-title" className="visually-hidden">
        Frise interactive du voyage
      </h3>
      <span className="trip-preview-center-guide" style={{ color: selectedPointColor }} aria-hidden="true">
        <ChevronDown size={20} />
        <ChevronUp size={20} />
      </span>
      <div
        ref={trackRef}
        className="trip-preview-viewport"
        onPointerDown={startTimelineDrag}
        onPointerMove={moveTimelineDrag}
        onPointerUp={(event) => finishTimelineDrag(event)}
        onPointerCancel={(event) => finishTimelineDrag(event, true)}
      ...
      >
        <ol className="trip-preview-track">
          {firstDay && (
            <li className="trip-preview-anchor-item">
              <button
                className={`trip-preview-anchor trip-preview-anchor--terminal${selectedKey === "departure" ? " is-selected" : ""}${selectedKey === "departure" && firstDay.stops[0] ? " has-active-connector" : ""}`}
                style={
                  {
                    "--trip-preview-color": firstDay.color ?? "#0FA68A",
                  } as CSSProperties
                }
                type="button"
                data-preview-navigation-index={navigationItems.findIndex((item) => item.key === "departure")}
                aria-label={`Départ${trip.departure ? ` : ${trip.departure.name}` : ""}`}
                aria-current={selectedKey === "departure" ? "step" : undefined}
                disabled={!trip.departure}
                onClick={() => {
                  if (trip.departure) onSelectLocation("departure", firstDay.id, null);
                }}
              >
                <strong>Départ</strong>
                <span className="trip-preview-anchor-dot">
                  <Play size={14} />
                </span>
                <small>{trip.departure?.name ?? "Non défini"}</small>
                <em>{formatClock(trip.departure?.departure_time ?? null)}</em>
              </button>
            </li>
          )}
          {trip.days.map((day, dayIndex) => {
            ...
            return (
              <Fragment key={day.id}>
                <li
                  className={`trip-preview-day-group${day.stops.length === 0 ? " is-empty" : ""}${daySelected || containsSelectedStop ? " is-active" : ""}`}
                  data-preview-day-id={day.id}
                  style={
                    {
                      "--trip-preview-color": dayColor,
                      "--trip-preview-next-color": nextDayColor,
                    } as CSSProperties
                  }
                >
                  ...
                  <span className="trip-preview-stop-segment" aria-label={`Étapes du jour ${day.day_number}`}>
                    {day.stops.length === 0 && (
                      <span className="trip-preview-empty-warning" role="img" aria-label={`Jour ${day.day_number} sans étape`} title="Aucune étape">
                    ...
                    {day.stops.map((stop, stopIndex) => {
                      ...
                      return (
                        <Fragment key={stop.id}>
                          <button
                            className={`trip-preview-stop${stopSelected ? " is-selected" : ""}${stopSelected && routeTarget ? " has-active-connector" : ""}`}
                            style={
                              {
                                "--trip-preview-color": dayColor,
                              } as CSSProperties
                            ...
                            type="button"
                            data-preview-navigation-index={navigationItems.findIndex((item) => item.key === stopKey)}
                            aria-label={`Étape ${stopIndex + 1} : ${stop.name}`}
                            aria-current={stopSelected ? "step" : undefined}
                            onClick={() => onSelectLocation(stopKey, day.id, stop.id)}
                          >
                  ...
                  <span className="trip-preview-day-label" aria-hidden="true">
                    Jour {day.day_number}
                  </span>
                </li>
                {nextDay && (
                  <li className="trip-preview-anchor-item">
                    {night ? (
                      <button
                        className={`trip-preview-anchor trip-preview-anchor--night${selectedKey === `night:${night.id}` ? " is-selected" : ""}${selectedKey === `night:${night.id}` && nextDay.stops[0] ? " has-active-connector" : ""}`}
                        style={
                          {
                            "--trip-preview-color": nextDayColor,
                            "--trip-preview-night-previous-color": dayColor,
                            "--trip-preview-night-next-color": nextDayColor,
                          } as CSSProperties
                        }
                        type="button"
                        data-preview-navigation-index={navigationItems.findIndex((item) => item.key === `night:${night.id}`)}
                        aria-label={`Nuit ${dayIndex + 1} : ${night.name}`}
                        aria-current={selectedKey === `night:${night.id}` ? "step" : undefined}
                        onClick={() => onSelectNight(night)}
                      >
                        <strong>Nuit {dayIndex + 1}</strong>
                        <span className="trip-preview-anchor-dot">
                          <Moon size={14} />
                        </span>
                        <small aria-hidden="true" />
                        {(night.check_in_from_time ?? night.check_in_time) && <em>{formatClock(night.check_in_from_time ?? night.check_in_time!)}</em>}
                      </button>
                    ) : (
                      <span className="trip-preview-anchor trip-preview-anchor--night trip-preview-anchor--missing" role="img" aria-label={`Nuit ${dayIndex + 1} non renseignée`} title="Nuit non renseignée">
                        <strong>Nuit {dayIndex + 1}</strong>
                        <span className="trip-preview-anchor-dot">
          ...
          {lastDay && (
            <li className="trip-preview-anchor-item">
              <button
                className={`trip-preview-anchor trip-preview-anchor--terminal${selectedKey === "arrival" ? " is-selected" : ""}`}
                type="button"
                data-preview-navigation-index={navigationItems.findIndex((item) => item.key === "arrival")}
                aria-label={`Arrivée${effectiveArrival ? ` : ${effectiveArrival.name}` : ""}`}
                aria-current={selectedKey === "arrival" ? "step" : undefined}
                disabled={!effectiveArrival}
                onClick={() => {
                  if (effectiveArrival) onSelectLocation("arrival", lastDay.id, null);
                }}
              >
                ...
                <small>{effectiveArrival?.name ?? "Non définie"}</small>
                <em>{arrivalTime}</em>
              </button>
            </li>
          )}
      ...
      {selectedLeg && (
        <div className="trip-preview-leg-card" style={{ "--trip-preview-color": selectedLeg.color } as CSSProperties} role="status" aria-label={`${selectedLeg.from} vers ${selectedLeg.to} : ${formatRouteDistance(selectedLeg.segment?.distance_meters ?? null)}, ${formatRouteDuration(selectedLeg.segment?.duration_seconds ?? null)}`}>
          <span>
            <small>Départ</small>
            <strong title={selectedLeg.from}>{selectedLeg.from}</strong>
          </span>
          <ChevronRight className="trip-preview-leg-card__separator" aria-hidden="true" size={16} />
          <span className="trip-preview-leg-card__metrics">
            <span className="trip-preview-leg-card__metric-values">
              <span>
                <Road aria-hidden="true" size={16} />
                {formatRouteDistance(selectedLeg.segment?.distance_meters ?? null)}
              </span>
              <i aria-hidden="true" />
              <span>
                <Clock3 aria-hidden="true" size={16} />
                {formatRouteDuration(selectedLeg.segment?.duration_seconds ?? null)}
              </span>
            </span>
          </span>
          <ChevronRight className="trip-preview-leg-card__separator" aria-hidden="true" size={16} />
          <span>
            <small>Arrivée</small>
            <strong title={selectedLeg.to}>{selectedLeg.to}</strong>
          </span>
        </div>
      )}
    </section>
...
function OptimizationMetrics({ value }: { value: TripOptimization }) {
  return (
    <div className="trip-optimization-comparison" aria-label="Comparaison de l’optimisation">
      <div>
        <strong>Avant</strong>
        <span>Distance : {formatRouteDistance(value.before_distance_meters)}</span>
        <span>Conduite : {formatRouteDuration(value.before_duration_seconds)}</span>
      </div>
      <div>
        <strong>Après</strong>
        <span>Distance : {formatRouteDistance(value.after_distance_meters)}</span>
        <span>Conduite : {formatRouteDuration(value.after_duration_seconds)}</span>
      </div>
      <div>
        <strong>Gain</strong>
        <span>Distance : {formatRouteDistance(value.distance_gain_meters)}</span>
        <span>Conduite : {formatRouteDuration(value.duration_gain_seconds)}</span>
      </div>
    </div>
  );
}
...
function GlobalOptimizationReview({ proposals, busy, onCancel, onApply }: { proposals: TripOptimization[]; busy: boolean; onCancel: () => void; onApply: () => void }) {
  ...
  return (
    <section className="trip-panel-global-optimization" aria-live="polite" aria-labelledby="global-optimization-title">
      <header>
        <div>
          <p className="cv-workspace-panel__eyebrow">Optimisation prête</p>
          <h3 id="global-optimization-title">
            Résultat pour {proposals.length} {proposals.length > 1 ? "journées" : "journée"}
          </h3>
        </div>
        <span>
          Gain total : {formatRouteDistance(totalDistanceGain)} · {formatRouteDuration(totalDurationGain)}
        </span>
      </header>
      <ul>
        {proposals.map((proposal) => (
          <li key={proposal.day_id}>
            <strong>Jour {proposal.day_number}</strong>
            <OptimizationMetrics value={proposal} />
          </li>
        ))}
      </ul>
      <footer>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt>{label}</dt>
      <dd aria-label={`${label} : ${value}`}>{value}</dd>
    </div>
  );
}
...
function InsertDayControl({ day, onInsert }: { day: TripDay; onInsert: () => void }) {
  return (
    <div className="trip-panel-insert-day">
      <button type="button" aria-label={`Ajouter une journée après le jour ${day.day_number}`} title={`Insérer une journée après le jour ${day.day_number}`} onClick={onInsert}>
        <span aria-hidden="true" />
        <i>
...
function DayColorPicker({ day, disabled, onSave }: { day: TripDay; disabled: boolean; onSave: (color: string) => void }) {
  ...
  return (
    <section className="trip-day-color-picker">
      ...
      <input id={`day-color-${day.id}`} aria-label={`Couleur du jour ${day.day_number}`} type="color" value={draftColor} disabled={disabled} onChange={(event) => setDraftColor(event.target.value.toUpperCase())} />
      <div className="trip-day-color-picker__actions">
        <button type="button" disabled={disabled || !hasChanges} onClick={() => setDraftColor(initialColor)}>
          Annuler
        </button>
        <button className="primary" type="button" disabled={disabled || !hasChanges} onClick={() => onSave(draftColor)}>
    ...
    <section id={`trip-day-settings-${day.id}`} className="trip-day-settings" aria-label={`Paramètres du jour ${day.day_number}`} hidden={!open}>
      <div className="trip-day-settings__body">
        <section className="trip-day-color-section">
          ...
          <DayColorPicker day={day} disabled={!canEdit || busy} onSave={onColorSave} />
        </section>
...
function DayVisibilityBubble({ day, hidden, onChange }: { day: TripDay; hidden: boolean; onChange: (visible: boolean) => void }) {
  ...
  return (
    <button
      className="trip-timeline-day-badge trip-day-visibility-bubble"
      type="button"
      role="switch"
    ...
    >
      <span className="trip-day-bubble__default" aria-hidden="true">
        <Sun size={12} />
      </span>
...
type TimelineCollapseRequest = { collapsed: boolean; expandedItem: string | null; version: number };
...
function PlaceDropGhost({ replacement = false, moving = false }: { replacement?: boolean; moving?: boolean }) {
  return <span className={`trip-place-drop-ghost${replacement ? " trip-place-drop-ghost--replacement" : ""}`} aria-hidden="true"><Plus size={14} />{replacement ? "Remplacer le POI" : moving ? "Déplacer l’étape ici" : "Déposer le POI ici"}</span>
}
...
function Departure({ trip, selected, recommendedStart, recommendedStartOffset, collapseRequest, onSelect, onStopFocus, onStopPlaceSelect }: { trip: Trip; selected: boolean; recommendedStart: string | null; recommendedStartOffset: number | null; collapseRequest: TimelineCollapseRequest; onSelect: () => void; onStopFocus?: (latitude: number, longitude: number) => void; onStopPlaceSelect: (placeId: string) => void }) {
  ...
  useEffect(() => setCollapsed(collapseRequest.collapsed || (collapseRequest.expandedItem !== null && collapseRequest.expandedItem !== "departure")), [collapseRequest]);
  ...
  const removeDeparture = (event: ReactMouseEvent) => {
    ...
    void runUndoable("suppression du point de départ", async () => {
      await deleteTripDeparture(departure.id);
      await anchorActions.reload(trip.id);
    });
  };
  ...
  return (
    <>
      <div
        ...
        onDragEnter={(event) => {
          if (anchorActions?.canEdit && hasDraggedPlace(event.dataTransfer)) {
            event.preventDefault();
            setDropActive(true);
          }
        }}
      ...
      >
        ...
        <div className="trip-night-content">
          <div
            className="trip-night-header-row"
            role="button"
            tabIndex={0}
            aria-pressed={selected}
            aria-label="Sélectionner le départ comme cible d’ajout"
            onClick={(event) => {
              if (!(event.target as HTMLElement).closest("button")) onSelect();
            }}
            onKeyDown={(event) => {
              if (!(event.target as HTMLElement).closest("button") && (event.key === "Enter" || event.key === " ")) {
                event.preventDefault();
                onSelect();
              }
            }}
          ...
          {dropActive && !departure && <PlaceDropGhost />}
          {!collapsed &&
            (departure ? (
              <div
                className="trip-night-stop"
                role="button"
                tabIndex={0}
                aria-label={departure.place_id ? "POI" : "Point cartographique"}
                onClick={focusDeparture}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    focusDeparture();
              ...
              >
                ...
                <span className="trip-night-stop-copy">
                  ...
                  <small>{departure.place_id ? "POI" : "Point cartographique"}</small>
                </span>
                {(!departure.place_id || anchorActions?.canEdit) && (
                  <span className="trip-night-stop-actions">
                    {!departure.place_id && (
                      <a className="trip-night-google-link" href={googleMapsAnchorUrl(departure)} target="_blank" rel="noopener noreferrer" aria-label={`Ouvrir ${departure.name} dans Google Maps`} title="Ouvrir dans Google Maps" onClick={(event) => event.stopPropagation()}>
                        <GoogleMapsIcon size={20} />
                      </a>
                    )}
                    {anchorActions?.canEdit && (
                      <button className="trip-night-remove" type="button" aria-label="Supprimer le point de départ" title="Supprimer le point de départ" onClick={removeDeparture}>
                        <Trash2 size={11} />
                      </button>
                    )}
                  </span>
                )}
                {dropActive && <PlaceDropGhost replacement />}
              </div>
            ) : (
...
function Arrival({ trip, selected, estimatedArrival, estimatedArrivalOffset, collapseRequest, onSelect, onStopFocus, onStopPlaceSelect }: { trip: Trip; selected: boolean; estimatedArrival: string | null; estimatedArrivalOffset: number | null; collapseRequest: TimelineCollapseRequest; onSelect: () => void; onStopFocus?: (latitude: number, longitude: number) => void; onStopPlaceSelect: (placeId: string) => void }) {
  ...
  const effectiveArrival = arrival;
  const reload = anchorActions?.reload ?? (async () => undefined);
  const runUndoable = useTripUndo(trip, reload);
  const estimatedArrivalLabel = formatClock(estimatedArrival, estimatedArrivalOffset);
  useEffect(() => setCollapsed(collapseRequest.collapsed || (collapseRequest.expandedItem !== null && collapseRequest.expandedItem !== "arrival")), [collapseRequest]);
  const focusArrival = () => {
    onSelect();
  ...
  const removeArrival = (event: ReactMouseEvent) => {
    ...
    void runUndoable("suppression du point d’arrivée", async () => {
      await deleteTripArrival(arrival.id);
      await anchorActions.reload(trip.id);
    });
  };
      ...
      >
        ...
        <div className="trip-night-content">
          <div
            ...
            tabIndex={0}
            aria-pressed={selected}
            aria-label="Sélectionner l’arrivée comme cible d’ajout"
            onClick={(event) => {
              if (!(event.target as HTMLElement).closest("button")) onSelect();
            }}
            onKeyDown={(event) => {
          ...
          {dropActive && !arrival && <PlaceDropGhost />}
          {!collapsed &&
            (effectiveArrival ? (
              <div
                className="trip-night-stop"
                role="button"
                tabIndex={0}
                aria-label={arrival ? (arrival.place_id ? "POI" : "Point cartographique") : "Même point que le départ"}
                onClick={focusArrival}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    focusArrival();
              ...
              >
                <MapPin aria-hidden="true" size={14} />
                <span className="trip-night-stop-copy">
                  <strong>{effectiveArrival.name}</strong>
                  <small>{arrival ? (arrival.place_id ? "POI" : "Point cartographique") : "Même point que le départ"}</small>
                </span>
                {arrival && (!arrival.place_id || anchorActions?.canEdit) && (
                  <span className="trip-night-stop-actions">
                    {!arrival.place_id && (
                      <a className="trip-night-google-link" href={googleMapsAnchorUrl(arrival)} target="_blank" rel="noopener noreferrer" aria-label={`Ouvrir ${arrival.name} dans Google Maps`} title="Ouvrir dans Google Maps" onClick={(event) => event.stopPropagation()}>
                        <GoogleMapsIcon size={20} />
                      </a>
                    )}
                    {anchorActions?.canEdit && (
                      <button className="trip-night-remove" type="button" aria-label="Supprimer le point d’arrivée" title="Supprimer le point d’arrivée" onClick={removeArrival}>
                        <Trash2 size={11} />
                      </button>
                    )}
                  </span>
                )}
                {dropActive && arrival && <PlaceDropGhost replacement />}
              </div>
            ) : (
              <div className="trip-night-placeholder">
...
function Night({ trip, previous, next, recommendedStart, recommendedStartOffset, activeTarget, canEdit, reload, collapseRequest, onSelect, onStopFocus }: { trip: Trip; previous: TripDay; next: TripDay; recommendedStart: string | null; recommendedStartOffset: number | null; activeTarget: TripNightTarget | null; canEdit: boolean; reload: (id?: string) => Promise<void>; collapseRequest: TimelineCollapseRequest; onSelect: (target: TripNightTarget, openPopup?: boolean) => void; onStopFocus?: (latitude: number, longitude: number) => void }) {
  ...
  const [dropActive, setDropActive] = useTransientDropState();
  const [collapsed, setCollapsed] = useState(false);
  useEffect(() => setCollapsed(collapseRequest.collapsed || (collapseRequest.expandedItem !== null && collapseRequest.expandedItem !== `night:${previous.id}:${next.id}`)), [collapseRequest, previous.id, next.id]);
    ...
    void (async () => {
      try {
        await runUndoable(night ? "modification de la nuit" : "ajout de la nuit", async () => {
          if (night)
            await updateTripNight(night.id, {
              place_id: placeId,
              source_type: "place",
            });
          else
            await addTripNight(trip.id, {
              previous_day_id: previous.id,
              next_day_id: next.id,
              place_id: placeId,
              source_type: "place",
            });
  ...
  const timelineColors = {
    "--trip-night-previous-color": previous.color ?? "#0FA68A",
    "--trip-night-next-color": next.color ?? "#0FA68A",
  } as CSSProperties;
  const selection = {
    nightId: night?.id ?? null,
    previousDayId: previous.id,
    nextDayId: next.id,
  };
  ...
  const removeNightLocation = () => {
    if (!night) return;
    void runUndoable("suppression du lieu de nuit", async () => {
      await deleteTripNight(night.id);
      await reload(trip.id);
    });
  };
    ...
    <>
      ...
      <div
        ...
        onClick={(event) => {
          ...
          const openPopup = Boolean(target.closest(".trip-night-stop"));
          selectNight(openPopup);
          if (openPopup) focusNightLocation();
        }}
        onKeyDown={(event) => {
          if (event.key === "Enter" || event.key === " ") {
            event.preventDefault();
        ...
        onDragEnter={(event) => {
          if (canEdit && hasDraggedPlace(event.dataTransfer)) {
            setCollapsed(false);
            setDropActive(true);
          }
        }}
      ...
      >
        ...
        <div className="trip-night-content">
          ...
          {!collapsed &&
            (night ? (
              <div className="trip-night-stop" aria-label={nightSourceLabel(night.source_type, night.place_id)}>
                <MapPin className="trip-stop-kind" aria-hidden="true" size={14} />
                <span className="trip-night-stop-copy">
                ...
                {canEdit && (
                  <span className="trip-night-stop-actions">
                    {canEdit && (
                      <button className="trip-night-remove" type="button" aria-label="Retirer le lieu de la nuit" title="Retirer le lieu" onClick={removeNightLocation}>
                        <Trash2 size={11} />
                      </button>
                    )}
                  </span>
              ...
              <div className="trip-night-placeholder">
                {dropActive ? (
                  ...
                  <span>Glissez un POI ou utilisez la recherche de la carte</span>
                )}
              </div>
            ))}
        </div>
...
function getDayTimelineStatus(day: TripDay, summary: TripDayTimeSummary | undefined): TimelineStatus {
  ...
  const constraintNeedsAttention = summary?.country_constraint_status === "unchecked" || summary?.country_constraint_status === "invalid" || summary?.country_constraint_status === "unavailable";
...
function canCalculateRoute(trip: Trip, day: TripDay, dayIndex: number) {
  const hasInheritedStart = dayIndex > 0 && trip.days[dayIndex - 1]?.stops.length > 0;
  const hasStart = dayIndex === 0 ? trip.departure !== null : trip.nights.some((night) => night.next_day_id === day.id) || hasInheritedStart;
  const hasEnd = trip.nights.some((night) => night.previous_day_id === day.id) || (dayIndex === trip.days.length - 1 && trip.arrival != null);
  return day.stops.length + Number(hasStart) + Number(hasEnd) >= 2;
}
function readLoadSettings(trip: Trip): TripLoadSettings {
  return {
    low_load_max_minutes: trip.low_load_max_minutes,
    medium_load_max_minutes: trip.medium_load_max_minutes,
    low_load_color: trip.low_load_color,
    medium_load_color: trip.medium_load_color,
    high_load_color: trip.high_load_color,
  };
}
