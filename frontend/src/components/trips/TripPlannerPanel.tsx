...
export function TripPlannerPanel({ poiMap, trip, activeDayId, tripViewOnly = false, hiddenDayIds = new Set<string>(), collapsed = false, createRequest = 0, onCollapsedChange = () => undefined, onTripViewOnlyChange = () => undefined, onDayVisibilityChange = () => undefined, onTripChange, onActiveDayChange, onActiveNightTargetChange = () => undefined, onStopFocus, onStopPlaceSelect = () => undefined }: Props) {
  ...
  return <aside className={`map-sidebar trip-planner-panel${tripViewOnly ? ' trip-planner-panel--trip-view' : ''}${isArchivedTrip ? ' trip-planner-panel--read-only' : ''}${collapsed ? ' is-collapsed' : ''}`} aria-label="Préparation de sortie">
    ...
    {tripViewOnly ? <div className="trip-panel-compact-summary">{summary && trip ? <TripSummaryMetrics trip={trip} summary={summary} defaultOpen canEdit={canEditTrip} busy={busy} onStartDateChange={(startDate) => void run(async () => { await updateTrip(trip.id, { start_date: startDate }); await reload(trip.id) })} /> : <div className="trip-panel-empty" role="status"><Route size={24} /><strong>Chargement du résumé…</strong></div>}</div> : <>
    ...
    {!trip ? <EmptyState className="trip-panel-empty" icon={<Route size={28} />} title="Aucune sortie préparée" description="Créez un voyage puis ajoutez les POI depuis le panneau Lieux." /> : <>
      ...
      {summary && <TripSummaryMetrics trip={trip} summary={summary} canEdit={canEditTrip} busy={busy} onStartDateChange={(startDate) => void run(async () => { await updateTrip(trip.id, { start_date: startDate }); await reload(trip.id) })} />}
...
function TripSummaryMetrics({ trip, summary, defaultOpen = false, canEdit, busy, onStartDateChange }: { trip: Trip; summary: TripSummary; defaultOpen?: boolean; canEdit: boolean; busy: boolean; onStartDateChange: (value: string | null) => void }) {
  return <section className="trip-summary-shell">
    ...
    <div className="trip-summary-dates" aria-label="Dates du voyage">
      <label><CalendarDays aria-hidden="true" size={16} /><span><small>Date de départ</small>{canEdit ? <input aria-label="Date de départ du voyage" type="date" value={trip.start_date ?? ''} disabled={busy} onChange={(event) => onStartDateChange(event.target.value || null)} /> : <strong>{formatTripDate(trip.start_date)}</strong>}</span></label>
      <div><Flag aria-hidden="true" size={16} /><span><small>Date d’arrivée</small><strong>{formatTripDate(trip.end_date)}</strong></span></div>
    </div>
