...
describe('CreateTripDialog', () => {
  ...
  it('creates a trip from an integrated accessible modal', async () => {
    ...
    render(<CreateTripDialog maps={[{ id: 'map-belgium', name: 'Belgique' }, { id: 'map-france', name: 'France' }] as never} onClose={vi.fn()} onCreate={onCreate} />)
    expect(screen.getByRole('dialog', { name: 'Préparer une sortie' })).toBeVisible()
    fireEvent.change(screen.getByLabelText('Carte associée *'), { target: { value: 'map-france' } })
    ...
    await waitFor(() => expect(onCreate).toHaveBeenCalledWith('map-france', { name: 'Ardennes', description: undefined, start_date: '2026-08-10', routing_profile: 'walking' }))
  })

  it('keeps the modal open and shows a compact error if creation fails', async () => {
    render(<CreateTripDialog maps={[{ id: 'map-belgium', name: 'Belgique' }] as never} onClose={vi.fn()} onCreate={vi.fn().mockRejectedValue(new Error('Internal Server Error'))} />)
