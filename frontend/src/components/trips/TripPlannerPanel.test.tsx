...
describe('TripPlannerPanel', () => {
  ...
  it('navigates the interactive preview timeline and focuses its stops on the map', async () => {
    ...
    const timelineLegCard = screen.getByLabelText('Musée vers Hôtel Central : 4,2 km, 8 min')
    vi.spyOn(timelineLegCard, 'getBoundingClientRect').mockReturnValue({ x: 130, y: 100, top: 100, right: 230, bottom: 150, left: 130, width: 100, height: 50, toJSON: () => ({}) })
    ...
    await waitFor(() => expect(centerScroll).toHaveBeenCalledWith({ left: 770, behavior: 'smooth' }))
    await waitFor(() => expect(timelineViewport.scrollLeft).toBe(770))
