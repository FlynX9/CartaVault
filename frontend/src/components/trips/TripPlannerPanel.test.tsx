...
describe('TripPlannerPanel', () => {
  ...
  it('toggles the map visibility of each day independently', async () => {
    ...
    expect(hideToggle).toHaveClass('trip-timeline-day-badge', 'trip-day-visibility-bubble')
  ...
  it('separates daily and global route metrics from visits', async () => {
    ...
    expect(screen.getByLabelText('Résumé de la journée')).toHaveTextContent('Temps de route')
    expect(screen.getByLabelText('Résumé de la journée')).toHaveTextContent('Temps total')
  ...
  it('groups day planning and color settings and shows the recommended time on the preceding anchor', async () => {
    ...
    expect(container.querySelector('.trip-timeline-day-badge .lucide-sun')).toBeInTheDocument()
    expect(screen.queryByText('Bilan de la journée')).not.toBeInTheDocument()
