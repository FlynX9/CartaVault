      ...
      <section className="kmz-import-dialog cv-modal" role="dialog" aria-modal="true" aria-labelledby="kmz-import-title">
        ...
        {step === 'preview' && preview && <div className="kmz-import-preview">
          ...
          <ul className="kmz-import-items">{preview.items.map((item) => {
            ...
            return <li key={item.source_index} className={`${!item.importable ? 'invalid' : ''}${item.outside_map_country ? ' outside-country' : ''}`.trim()}>
              ...
              {item.outside_map_country && <small className="kmz-import-country-warning"><ShieldAlert size={14} />Hors du pays de la carte — sélection manuelle requise</small>}
