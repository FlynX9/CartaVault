...
import type { KmzImportReport, KmzPreview } from '../../types/imports'
    ...
    <div className="cv-overlay kmz-import-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <section className="kmz-import-dialog cv-modal" role="dialog" aria-modal="true" aria-labelledby="kmz-import-title">
        ...
        {step === 'preview' && preview && <div className="kmz-import-preview">
          ...
          <div className="kmz-import-toolbar"><button type="button" onClick={toggleAll}><CheckSquare size={16} />Tout sélectionner</button><button type="button" onClick={() => { setSelected([]); setForced([]) }}>Tout désélectionner</button></div>
        ...
        {step === 'confirming' && <p className="kmz-import-state" role="status">Import en cours…</p>}
