...
describe('KmzImportDialog', () => {
  ...
  it('requires an explicit selection for a point outside the map country', async () => {
    vi.mocked(previewKmzImport).mockResolvedValue({
      ...preview,
      warning_count: 1,
      items: [{
        ...
        outside_map_country: true,
        warnings: ['Ce point semble situé hors de Belgique.'],
      }],
    })
    render(<KmzImportDialog poiMap={poiMap} onClose={vi.fn()} onImported={vi.fn()} />)
    const input = document.body.querySelector('input[type="file"]') as HTMLInputElement
    ...
    await screen.findByText('Hors du pays de la carte — sélection manuelle requise')
    const checkbox = document.body.querySelector('.kmz-import-items input[type="checkbox"]') as HTMLInputElement
    expect(checkbox).not.toBeChecked()
    expect(checkbox).toBeEnabled()
    fireEvent.click(checkbox)
    expect(screen.getByRole('button', { name: /Importer 1 POI/ })).toBeEnabled()
  })
