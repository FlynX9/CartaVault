...
function ServiceDialog({ kind, initial, keys, stadiaKeyOptional, onClose, onSave }: { kind: ServiceKind; initial: ServiceDraft; keys: PersonalApiKey[]; stadiaKeyOptional: boolean; onClose: () => void; onSave: (draft: ServiceDraft) => void }) {
  ...
  const changeProvider = (provider: string) => setDraft({ ...draft, provider, apiKeyId: '' })
  ...
  return createPortal(<div className="account-integration-dialog-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className="account-integration-dialog" role="dialog" aria-modal="true" aria-labelledby="account-integration-dialog-title">
      ...
      <div className="account-integration-dialog__content">
        ...
        {kind === 'satellite-basemap' && draft.provider === 'google' && <label>Intégration Google Satellite<select value={draft.googleSatelliteMode} onChange={(event) => setDraft({ ...draft, googleSatelliteMode: event.target.value as ServiceDraft['googleSatelliteMode'], apiKeyId: '' })}><option value="maps-js">Maps JavaScript API — compatible EEE</option><option value="map-tiles">Map Tiles API — selon disponibilité Google</option></select></label>}
...
export function IntegrationPreferences({ preferences, setPreferences }: { preferences: AccountPreferences; setPreferences: (value: AccountPreferences) => void }) {
  ...
  const providerKey = (provider: string, kind: ServiceKind, mode = googleSatelliteMode) => provider === 'stadia' ? basemaps.stadia_api_key_id : provider === 'google' ? (kind === 'satellite-basemap' && mode === 'maps-js' ? basemaps.google_maps_js_api_key_id : basemaps.google_api_key_id) : provider === 'mapbox' ? basemaps.mapbox_api_key_id : null
  const initialDraft: ServiceDraft = editing === 'routing'
    ? { provider: preferences.routing.provider, apiKeyId: preferences.routing.api_key_id ?? '', googleSatelliteMode }
    : editing === 'places'
      ? { provider: preferences.places.provider, apiKeyId: preferences.places.api_key_id ?? '', googleSatelliteMode }
      : editing === 'classic-basemap' ? { provider: classicProvider, apiKeyId: providerKey(classicProvider, 'classic-basemap') ?? '', googleSatelliteMode } : { provider: basemaps.satellite_provider, apiKeyId: providerKey(basemaps.satellite_provider, 'satellite-basemap') ?? '', googleSatelliteMode }
  ...
  const saveService = (draft: ServiceDraft) => {
    ...
    if (editing === 'satellite-basemap') { const provider = draft.provider as NonNullable<AccountPreferences['basemaps']>['satellite_provider']; setPreferences({ ...preferences, preferred_basemap: provider === 'google' ? (draft.googleSatelliteMode === 'map-tiles' ? 'google-satellite-tiles' : 'google-satellite') : preferences.preferred_basemap, basemaps: { ...basemaps, satellite_provider: provider, google_satellite_mode: draft.googleSatelliteMode, ...(provider === 'stadia' ? { stadia_api_key_id: draft.apiKeyId || null } : provider === 'google' ? (draft.googleSatelliteMode === 'maps-js' ? { google_maps_js_api_key_id: draft.apiKeyId || null } : { google_api_key_id: draft.apiKeyId || null }) : provider === 'mapbox' ? { mapbox_api_key_id: draft.apiKeyId || null } : {}) } }) }
    setEditing(null)
  }
  ...
  return <>
    <section className="account-preference-card account-integration-preferences">
      ...
      <div className="account-integration-preferences__list">
        ...
        <ServiceRow icon={ImageIcon} title={t('account.integrations.classicBasemap')} description={t('account.integrations.classicBasemapDescription')} providerLabel={providerLabels[classicProvider]} providerCaption={t('account.integrations.provider')} keyName={classicProvider === 'osm' || (classicProvider === 'stadia' && stadiaKeyOptional && !providerKey('stadia', 'classic-basemap')) ? t('account.integrations.noKeyRequired') : keyLabel(classicKeys, providerKey(classicProvider, 'classic-basemap'), t('account.integrations.noKey'))} state={<ServiceState keys={classicKeys} value={providerKey(classicProvider, 'classic-basemap')} optional={classicProvider === 'osm' || (classicProvider === 'stadia' && stadiaKeyOptional)} />} onEdit={() => setEditing('classic-basemap')} />
        <ServiceRow icon={ImageIcon} title={t('account.integrations.satelliteBasemap')} description={basemaps.satellite_provider === 'google' ? (googleSatelliteMode === 'maps-js' ? 'Maps JavaScript API avec clé navigateur restreinte par référent HTTP.' : 'Map Tiles API avec clé serveur ; le satellite peut être indisponible pour une facturation EEE.') : t('account.integrations.satelliteBasemapDescription')} providerLabel={basemaps.satellite_provider === 'google' ? `Google Satellite · ${googleSatelliteMode === 'maps-js' ? 'Maps JavaScript' : 'Map Tiles'}` : basemaps.satellite_provider === 'none' ? t('account.integrations.disabled') : providerLabels[basemaps.satellite_provider]} providerCaption={t('account.integrations.provider')} keyName={basemaps.satellite_provider === 'none' || (basemaps.satellite_provider === 'stadia' && stadiaKeyOptional && !providerKey('stadia', 'satellite-basemap')) ? t('account.integrations.noKeyRequired') : keyLabel(satelliteKeys, providerKey(basemaps.satellite_provider, 'satellite-basemap'), t('account.integrations.noKey'))} state={<ServiceState keys={satelliteKeys} value={providerKey(basemaps.satellite_provider, 'satellite-basemap')} optional={basemaps.satellite_provider === 'none' || (basemaps.satellite_provider === 'stadia' && stadiaKeyOptional)} />} onEdit={() => setEditing('satellite-basemap')} />
      </div>
    </section>
