...
const typeMeta: Record<
  TrashItemType,
  {
    labelKey: "trash.type.map" | "trash.type.place" | "trash.type.trip";
    Icon: typeof MapPin;
  }
> = {
  ...
  useEffect(() => {
    ...
    void getTrash("all", controller.signal)
      ...
      .catch((caught: unknown) => {
        if (!(caught instanceof Error && caught.name === "AbortError")) setError(caught instanceof Error ? caught.message : t("trash.loadError"));
      })
      .finally(() => {
      ...
      {!collapsed && (
        <div className="trash-workspace-content cv-workspace-panel__content">
          <div className="trash-filter" role="group" aria-label={t("trash.filter")}>
            {(
              [
                ["all", "trash.filter.all"],
                ["map", "trash.filter.map"],
                ["place", "trash.filter.place"],
                ["trip", "trash.filter.trip"],
              ] as const
            ).map(([value, label]) => (
                ...
                return (
                  <li key={`${item.item_type}:${item.id}`}>
                    ...
                    <span className="trash-item-copy">
                      ...
                      <small>
                        {t(labelKey)}
                        {item.map_name && item.item_type !== "map" ? ` · ${item.map_name}` : ""}
                      </small>
                      <small>
                        {t("trash.deletedAt", {
                    ...
                    <span className="trash-item-actions">
                      <button type="button" disabled={busyId === item.id} aria-label={t("trash.restore", { name: item.name })} title={t("trash.restoreTitle")} onClick={() => void restore(item)}>
                        <ArchiveRestore size={16} />
                      </button>
                      <button className="danger" type="button" disabled={busyId === item.id} aria-label={t("trash.delete", { name: item.name })} title={t("trash.deleteTitle")} onClick={() => void remove(item)}>
                        <Trash2 size={16} />
                      </button>
                    </span>
                  </li>
