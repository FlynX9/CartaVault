...
export function CreateMapDialog({ onClose, onCreated }: CreateMapDialogProps) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      void getCountries(query.trim() || undefined, controller.signal)
        .then(setCountries)
        .catch((caught: unknown) => { if (!(caught instanceof Error && caught.name === 'AbortError')) setError(caught instanceof Error ? caught.message : 'Catalogue indisponible.') })
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (!countryId) return
  ...
  return (
    <dialog className="create-map-dialog" open aria-modal="true" aria-labelledby="create-map-title">
      <form onSubmit={(event) => void submit(event)}>
        ...
        <label className="form-field"><span>Rechercher un pays</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} autoFocus /></label>
        <div className="country-results" role="listbox" aria-label="Pays disponibles">
          {isLoading ? <p>Chargement…</p> : countries.map((country) => <button className={country.id === countryId ? 'selected' : ''} type="button" role="option" aria-selected={country.id === countryId} key={country.id} onClick={() => selectCountry(country)}><strong>{country.name}</strong><span>{country.iso_alpha2} · {country.iso_alpha3}</span></button>)}
        </div>
