import { Check, Download, LockKeyhole, Map, MapPin, Plus, Route, Search, Settings2, Share2, SquareChevronDown, SquareChevronUp, Trash2, Users, X } from 'lucide-react'
...
export function MapsWorkspacePanel({ maps, activeMapId, isLoading, errorMessage, onOpen, onDelete, onCreated, onExport = () => undefined, onMembers = () => undefined, onAccessChanged = () => undefined, collapsed = false, onCollapsedChange, onClose, createRequest = 0 }: MapsWorkspacePanelProps) {
  ...
  return <aside id="workspace-maps-panel" className={`country-place-panel workspace-management-panel cv-workspace-panel maps-workspace-panel${collapsed ? ' is-collapsed' : ''}`} aria-labelledby="workspace-maps-title" tabIndex={-1}>
    ...
    <div className="maps-workspace-panel__content">
      ...
      {!isLoading && totalCount === 0 && <EmptyState className="maps-panel-empty" icon={<Map size={28} />} title={t('maps.empty')} action={{ label: t('maps.create'), onClick: () => setCreating(true) }} />}
      ...
      <ul className="maps-catalog" aria-label={t('maps.available')}>
        {filteredInvitations.map((invitation) => <li className="maps-catalog__invitation" key={`invitation-${invitation.id}`}>
          ...
          <div className="maps-catalog__details"><span className="maps-catalog__privacy shared" aria-label={t('maps.invitation.shared')} title={t('maps.invitation.shared')}><Share2 size={15} /></span><strong>{invitation.map_name}</strong><span>{t('maps.invitation.by', { name: invitation.invited_by_display_name })}</span><em>{invitation.role === 'owner' ? t('maps.invitation.owner') : invitation.role === 'editor' ? t('maps.invitation.editor') : t('maps.invitation.viewer')}</em><b>{t('maps.invitation.pending')}</b></div>
