...
describe('MapPlaceList', () => {
  ...
  it('opens a list item and reflects marker selection', async () => { const place = { id: 'place-id', name: 'Manufacture', latitude: 48, longitude: 2, status: { id: 'status-id', name: 'À faire', slug: 'a-faire', color: '#2563EB', is_active: true }, categories: [], tags: [] } as never; vi.mocked(getPlaces).mockResolvedValue([place]); const select = vi.fn(); render(<MapPlaceList poiMap={{ id: 'map-id', name: 'France' } as never} selectedPlaceId="place-id" refreshVersion={0} removedPlaceId={null} onPlaceSelect={select} />); const item = await screen.findByRole('button', { name: /Manufacture/ }); expect(item).toHaveClass('selected'); expect(item).toHaveTextContent('À faire'); fireEvent.click(item); expect(select).toHaveBeenCalledWith(place) })
})
