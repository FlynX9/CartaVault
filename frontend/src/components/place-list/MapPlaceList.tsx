import { ArrowDownAZ, CalendarPlus, CheckSquare, FileInput, Heart, LayoutList, List, MapPinned, Minus, Pencil, Plus, Search, SlidersHorizontal, Trash2, X } from 'lucide-react'
import { Link } from 'react-router-dom'
import { useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'

import { bulkAddPlacesToTrip, bulkUpdatePlaces, deletePlace, getPlaceFacets, getPlaceListPosition, getPlaces, updatePlace } from '../../api/places'
...
interface Props {
  ...
  onFiltersChange?: (filters: PlaceFilters) => void; onPlaceSelect: (place: PreviewPlace) => void; onClose?: () => void; collapsed?: boolean; onCollapsedChange?: (collapsed: boolean) => void; onImported?: () => void; tripPlanningActive?: boolean; tripPlaceIds?: Set<string>; tripAddTargetLabel?: string | null; onTripPlaceAdd?: (place: PreviewPlace) => void; onBulkChanged?: () => void; onBulkTripChanged?: (tripId: string) => void; importRequest?: number
  selectionMode?: boolean; selectedPlaceIds?: ReadonlySet<string>; onSelectionModeChange?: (active: boolean) => void; onSelectedPlaceIdsChange?: (ids: Set<string>) => void
}
...
export function MapPlaceList({ poiMap, statuses = [], filters = DEFAULT_PLACE_FILTERS, selectedPlaceId, refreshVersion, removedPlaceId, onFiltersChange = () => undefined, onPlaceSelect, collapsed = false, onCollapsedChange = () => undefined, onImported = () => undefined, tripPlanningActive = false, tripPlaceIds = new Set(), tripAddTargetLabel = null, onTripPlaceAdd = () => undefined, onBulkChanged = () => undefined, onBulkTripChanged = () => undefined, importRequest = 0, selectionMode: controlledSelectionMode, selectedPlaceIds: controlledSelectedIds, onSelectionModeChange, onSelectedPlaceIdsChange }: Props) {
  ...
  const [filtersOpen, setFiltersOpen] = useState(false); const [internalSelectionMode, setInternalSelectionMode] = useState(false); const [internalSelectedIds, setInternalSelectedIds] = useState<Set<string>>(new Set()); const [bulkBusy, setBulkBusy] = useState(false); const [bulkError, setBulkError] = useState<string | null>(null); const [bulkNotice, setBulkNotice] = useState<string | null>(null)
  ...
  const [displayMode, setDisplayMode] = useState<'compact' | 'expanded'>('expanded'); const [bulkEditorOpen, setBulkEditorOpen] = useState(false)
  ...
  useEffect(() => { selectionController.current?.abort(); replaceSelectedIds(new Set()); setPlaces([]); setFacets(emptyFacets); setNextOffset(0); setListReady(false); setBulkNotice(null) }, [poiMap?.id, replaceSelectedIds])
  useEffect(() => {
  ...
  const toggleSelected = (id: string) => { const next = new Set(selectedIds); if (next.has(id)) next.delete(id); else next.add(id); replaceSelectedIds(next) }
  ...
  const togglePage = () => { const ids = visible.map((place) => place.id); const next = new Set(selectedIds); const every = ids.length > 0 && ids.every((id) => next.has(id)); ids.forEach((id) => every ? next.delete(id) : next.add(id)); replaceSelectedIds(next) }
    ...
    >
      <header className="places-redesign-header">
        ...
        <div className="places-redesign-header-actions">
          ...
          {!collapsed && poiMap && !tripPlanningActive && poiMap.can_edit !== false && <Link className="places-redesign-create panel-create-action" to={withMap('/places/new', poiMap.id)} aria-label={t('places.new')} title={t('places.new')}><Plus size={18} aria-hidden="true" /><span className="panel-create-action__label">{t('places.new')}</span></Link>}
      ...
      {poiMap && <section className="places-redesign-controls" aria-label={t('places.controls')}>
        ...
        <div className="places-redesign-toolbar">
          ...
          {poiMap && <button className={`panel-icon-button places-selection-toggle${selectionMode ? ' primary' : ''}`} type="button" aria-label={t('places.selection')} aria-pressed={selectionMode} onClick={() => { replaceSelectionMode(!selectionMode); replaceSelectedIds(new Set()) }}><CheckSquare size={17} /></button>}
