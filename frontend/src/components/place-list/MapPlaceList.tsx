...
export function MapPlaceList({ poiMap, statuses = [], filters = DEFAULT_PLACE_FILTERS, selectedPlaceId, refreshVersion, removedPlaceId, onFiltersChange = () => undefined, onPlaceSelect, collapsed = false, onCollapsedChange = () => undefined, onImported = () => undefined, tripPlanningActive = false, tripPlaceIds = new Set(), onBulkChanged = () => undefined }: Props) {
  ...
  const toggleQuickStatus = (name: string) => {
    const status = statuses.find((item) => item.name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('fr-FR') === name)
    if (status) update({ statusIds: filters.statusIds.length === 1 && filters.statusIds[0] === status.id ? [] : [status.id], isFavorite: null, isVisited: null })
  }
    ...
    <aside className={`country-place-panel cv-workspace-panel places-redesign-panel${collapsed ? ' is-collapsed' : ''}`} id="map-place-list" tabIndex={-1} aria-labelledby="map-place-list-title">
      ...
      {poiMap && <section className="places-redesign-controls" aria-label="Recherche et filtres des lieux">
        ...
        <nav className="places-quick-filters" aria-label="Filtres rapides">
          <button type="button" className={!hasActivePlaceFilters(filters) ? 'active' : ''} onClick={() => onFiltersChange(DEFAULT_PLACE_FILTERS)}>Tous</button>
          {(['a faire', 'visites', 'a verifier'] as const).map((name) => { const status = statuses.find((item) => item.name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('fr-FR') === name); const label = name === 'a faire' ? 'À faire' : name === 'visites' ? 'Visités' : 'À vérifier'; const active = status ? filters.statusIds.includes(status.id) : name === 'visites' ? filters.isVisited === true : false; return <button key={name} type="button" className={active ? 'active' : ''} onClick={() => name === 'visites' ? update({ isVisited: filters.isVisited === true ? null : true, statusIds: [], isFavorite: null }) : toggleQuickStatus(name)}>{label}{status && <small>{facets.statuses.find((facet) => facet.id === status.id)?.count ?? 0}</small>}</button> })}
          <button type="button" className={filters.isFavorite === true ? 'active' : ''} onClick={() => update({ isFavorite: filters.isFavorite === true ? null : true, statusIds: [], isVisited: null })}>Favoris<small>{visible.filter((place) => place.is_favorite).length}</small></button>
        </nav>
