...
import { Images, LayoutDashboard, PanelLeftClose, PanelLeftOpen, Trash2 } from 'lucide-react'
  ...
  return <nav className={`main-navigation cv-main-navigation${collapsed ? ' is-collapsed' : ''}${isMobileViewport ? ' is-mobile' : ''}`} aria-label={t('nav.main')}>
    ...
    <div className="main-navigation-links cv-main-navigation__items">
      <div className="cv-main-navigation__group">
        ...
        <button type="button" className={navClass(mapsActive)} aria-label={t('nav.maps')} aria-pressed={mapsActive} onClick={() => selectPanel('maps')}><IconVault className="cv-main-navigation__vault-icon" aria-hidden="true" size={23} stroke={2} /><span>{t('nav.maps')}</span></button>
