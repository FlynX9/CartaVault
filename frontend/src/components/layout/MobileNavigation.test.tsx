...
describe('MobileNavigation', () => {
  ...
  it('renders the validated map from MAP_MODE instead of activeMapId', () => {
    ...
    const menu = screen.getByRole('menu', { name: 'Actions de la carte' })
    expect(menu).toHaveAttribute('data-map-id', map.id)
    expect(within(menu).getAllByRole('menuitem')).toHaveLength(6)
    for (const name of ['Catégories', 'Tags', 'Statuts', 'Annotations', 'Médias de la carte', 'Paramètres de la carte']) {
      expect(within(menu).getByRole('menuitem', { name })).toHaveAttribute('data-map-id', map.id)
    }
    expect(screen.getByRole('button', { name: 'Plus' })).toHaveAttribute('aria-pressed', 'true')
    fireEvent.click(within(menu).getByRole('menuitem', { name: 'Catégories' }))
    expect(onMapNavigation).toHaveBeenLastCalledWith('categories', map.id)
    fireEvent.click(screen.getByRole('button', { name: 'Plus' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Tags' }))
    expect(onMapNavigation).toHaveBeenLastCalledWith('tags', map.id)
    fireEvent.click(screen.getByRole('button', { name: 'Plus' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Statuts' }))
    expect(onMapNavigation).toHaveBeenLastCalledWith('statuses', map.id)
    fireEvent.click(screen.getByRole('button', { name: 'Plus' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Annotations' }))
    expect(onMapNavigation).toHaveBeenLastCalledWith('annotations', map.id)
    fireEvent.click(screen.getByRole('button', { name: 'Plus' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Médias de la carte' }))
    expect(onMapNavigation).toHaveBeenLastCalledWith('media', map.id)
    fireEvent.click(screen.getByRole('button', { name: 'Plus' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Paramètres de la carte' }))
    expect(onMapNavigation).toHaveBeenLastCalledWith('settings', map.id)
    fireEvent.click(screen.getByRole('button', { name: 'Plus' }))
    fireEvent.pointerDown(document.body)
    expect(screen.queryByRole('menu', { name: 'Actions de la carte' })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Plus' }))
