...
interface TopBarProps {
  isMapWorkspace: boolean; maps: PoiMap[]; activeMapId: string | null; activeStatusId: string | null; areMapsLoading: boolean; mapsError: string | null; markerCount: number; placeListOpen: boolean
  onMapChange: (mapId: string | null) => void; onMapCreated: (poiMap: PoiMap) => void; onDeleteMap: () => void; onTogglePlaceList: () => void
}

export function TopBar({ isMapWorkspace, maps, activeMapId, activeStatusId, areMapsLoading, mapsError, markerCount, placeListOpen, onMapChange, onMapCreated, onDeleteMap, onTogglePlaceList }: TopBarProps) {
  ...
  return <header className="app-header">
    ...
    <nav className="app-header-actions" aria-label="Navigation principale">
      {isMapWorkspace && <>
        ...
        {activeMapId && <Link className="header-link" to={withMap('/places/new', activeMapId, activeStatusId)}>Ajouter un POI</Link>}
      </>}
      <Link className="header-link" to="/admin/categories">Administration</Link>
      {isMapWorkspace ? <div className="marker-count" aria-live="polite"><strong>{markerCount}</strong><span>marqueur{markerCount > 1 ? 's' : ''}</span></div> : <Link className="header-link" to={withMap('/', activeMapId, activeStatusId)}>Carte</Link>}
    </nav>
