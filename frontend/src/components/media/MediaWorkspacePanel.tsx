...
function MediaUploadDialog({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const [maps, setMaps] = useState<PoiMap[]>([]); const [mapId, setMapId] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null)
  useEffect(() => { const controller = new AbortController(); void getMaps(controller.signal).then((items) => { if (controller.signal.aborted) return; setMaps(items.filter((map) => map.can_edit)); setMapId((current) => current || items.find((map) => map.can_edit)?.id || '') }).catch((caught) => { if (controller.signal.aborted || (caught instanceof DOMException && caught.name === 'AbortError')) return; setError(caught instanceof Error ? caught.message : 'Cartes indisponibles.') }); return () => controller.abort() }, [])
  ...
  const submit = async (files: FileList | null) => {
    ...
    try { for (const source of Array.from(files)) { const [coordinates, compressed] = await Promise.all([readImageLocation(source), compressImage(source)]); await uploadMedia(compressed, mapId, coordinates, undefined, source) }; onDone(); onClose() }
