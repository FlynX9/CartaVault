...
const page: MediaPage = {
  ...
  filters: {
    ...
    formats: ["WEBP"],
    uploaders: [{ id: "user-1", name: "Alice" }],
  },
};
...
describe("MediaWorkspacePanel", () => {
  ...
  it("debounces search and applies server-side filters", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Search|Rechercher/i), {
      target: { value: "chapelle" },
    });
  ...
  it("maximizes its floating desktop window on request", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "Agrandir la fenêtre Médias au maximum",
      }),
    );
    ...
    const restore = screen.getByRole("button", {
      name: "Rétablir la taille précédente de la fenêtre Médias",
    });
