...
describe('PlaceMapPopup', () => {
  it('renders the structured inline detail without duplicating the row image or title', async () => {
    ...
    expect(within(details).getByRole('heading', { name: 'Informations pratiques' })).toBeVisible()
    ...
    const risk = within(details).getByText('Risque').closest('article') as HTMLElement
    expect(risk).toBeVisible()
    expect(risk.querySelector('.popup-danger-value')).toHaveStyle('--danger-level-color: #d94d4d')
  ...
  it('reveals the history directly inside the quick card', async () => {
    ...
    expect(await screen.findByText('Aucun changement enregistré.')).toBeVisible()
    expect(document.querySelector('.popup-history h3')).toHaveTextContent('Historique')
  })
  ...
  it('uses the compact card hierarchy and shows the first image from the file endpoint', async () => {
    ...
    expect(screen.getByText('Risque')).toBeVisible()
  ...
  it('previews and saves popup ratings in half-star increments', async () => {
    ...
    expect(screen.getByLabelText('Envie avant visite : 3.5 sur 5')).toBeVisible()
    expect(screen.getByText('3.5')).toBeVisible()
    expect(screen.getByLabelText('Envie avant visite : 3.5 sur 5').querySelector('[data-fill="50"]')).toBeInTheDocument()
