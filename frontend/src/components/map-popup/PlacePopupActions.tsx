...
interface Props { googleMapsUrl: string | null; isDeleting: boolean; canEdit?: boolean; showClose?: boolean; tripAddTargetLabel?: string | null; isAddingToTrip?: boolean; onAddToTrip?: () => void; onEdit: () => void; onDelete: () => void; onClose: () => void }

export function PlacePopupActions({ googleMapsUrl, isDeleting, canEdit = true, showClose = true, tripAddTargetLabel = null, isAddingToTrip = false, onAddToTrip = () => undefined, onEdit, onDelete, onClose }: Props) {
  return <div className="popup-actions" aria-label="Actions du POI">
    {tripAddTargetLabel && <button className="popup-action-add-to-trip" type="button" aria-label={`${tripAddTargetLabel}`} title={tripAddTargetLabel} disabled={isDeleting || isAddingToTrip} onClick={onAddToTrip}><CalendarPlus aria-hidden="true" size={17} /><span>{isAddingToTrip ? 'Ajout…' : 'Ajouter à la sortie'}</span></button>}
