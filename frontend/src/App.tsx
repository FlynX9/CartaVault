  ...
  const activeTripAddTargetLabel = tripPlannerOpen
    ...
    ? activeTripNightTarget === null
      ...
      : activeTripNightTarget.nightId === null
        ...
        : null
    : null
  const activeTripPlaceIds = new Set([...(activeTrip?.days.flatMap((day) => day.stops.map((stop) => stop.place_id)) ?? []), ...(activeTrip?.nights.map((night) => night.place_id) ?? [])].filter((id): id is string => id !== null))
