...
function WorkspaceApp() {
  ...
  const activeTripAddTargetLabel = tripPlannerOpen
    ...
    ? activeTripNightTarget === null
      ...
      : activeTripNightTarget.nightId === null
        ...
        : null
    : null
  const activeTripPlaceIds = useMemo(() => new Set(
    [
      ...(activeTrip?.days.flatMap((day) => day.stops.map((stop) => stop.place_id)) ?? []),
      ...(activeTrip?.nights.map((night) => night.place_id) ?? []),
    ].filter((id): id is string => id !== null),
  ), [activeTrip])
  const popupTripAddTargetLabel = selectedPlaceId !== null && !activeTripPlaceIds.has(selectedPlaceId) ? activeTripAddTargetLabel : null
  const popupContent = selectedPlaceId !== null && !editorOpen ? <PlaceMapPopup placeId={selectedPlaceId} canEdit={activeMap?.can_edit === true} tripAddTargetLabel={popupTripAddTargetLabel} onAddToTrip={addPlaceToActiveTripTarget} onEdit={() => navigate(withMap(`/places/${selectedPlaceId}/edit`, activeMapId, activeStatusId))} onDeleted={(id) => { handleDeletePlace(id); navigate(withMap('/', activeMapId, activeStatusId)) }} onClose={closePopup} /> : null
  const workspaceContent = <Suspense fallback={<aside className="cv-workspace-panel" role="status">Chargement du panneauâ€¦</aside>}>{workspacePanel === 'maps'
    ...
    ? <MapPlaceList poiMap={activeMap} statuses={statuses} filters={placeFilters} selectedPlaceId={selectedPlaceId} refreshVersion={refreshVersion} removedPlaceId={removedPlaceId} collapsed={placesPanelCollapsed} onCollapsedChange={setPlacesPanelCollapsed} onFiltersChange={(filters: PlaceFilters) => { const params = serializePlaceFilters(filters); if (activeMapId) params.set('map', activeMapId); navigate({ pathname: location.pathname, search: params.toString() ? `?${params}` : '' }) }} onPlaceSelect={(place) => handleSelect(place, true)} onImported={() => setRefreshVersion((value) => value + 1)} onBulkChanged={() => setRefreshVersion((value) => value + 1)} onBulkTripChanged={(tripId) => { if (activeTrip?.id === tripId) void getTrip(tripId).then(setActiveTrip).catch((caught: unknown) => showTripNotice(caught instanceof Error ? caught.message : 'Impossible de rafraîchir la sortie.')) }} selectionMode={placeSelectionMode} selectedPlaceIds={selectedPlaceIds} onSelectionModeChange={setPlaceSelectionMode} onSelectedPlaceIdsChange={setSelectedPlaceIds} tripPlanningActive={tripPlannerOpen} tripPlaceIds={activeTripPlaceIds} tripAddTargetLabel={activeTripAddTargetLabel} onTripPlaceAdd={(place) => void addPlaceToActiveTripTarget(place)} importRequest={importRequest} />
