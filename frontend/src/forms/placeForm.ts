...
export const EMPTY_PLACE_FORM_VALUES: PlaceFormValues = { name: '', mapId: '', statusId: '', description: '', region: '', construction_date: '', abandonment_date: '', condition: '', access: '', danger_level: '', latitude: '', longitude: '', categoryIds: [], primaryCategoryId: '', tagIds: [], isFavorite: false, interestRating: '', visitRating: '', visitDuration: '' }
...
export function placeDetailsToFormValues(place: PlaceDetails): PlaceFormValues { return { name: place.name, mapId: place.map_id, statusId: place.status.id, description: place.description ?? '', region: place.region ?? '', construction_date: place.construction_date ?? '', abandonment_date: place.abandonment_date ?? '', condition: place.condition ?? '', access: place.access ?? '', danger_level: place.danger_level ?? '', latitude: place.latitude?.toString() ?? '', longitude: place.longitude?.toString() ?? '', categoryIds: place.categories.map((category) => category.id), primaryCategoryId: place.categories.find((category) => category.is_primary)?.id ?? '', tagIds: place.tags.map((tag) => tag.id), isFavorite: place.is_favorite ?? false, interestRating: place.interest_rating?.toString() ?? '', visitRating: place.visit_rating?.toString() ?? '', visitDuration: place.default_visit_duration_minutes?.toString() ?? '' } }
