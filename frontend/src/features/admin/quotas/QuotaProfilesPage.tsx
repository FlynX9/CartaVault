...
export function QuotaProfilesPage() {
  const [profiles, setProfiles] = useState<QuotaProfile[]>([]); const [registry, setRegistry] = useState<QuotaRegistryItem[]>([]); const [selectedProfileId, setSelectedProfileId] = useState<string | null>(null); const [editing, setEditing] = useState<QuotaProfile | null | undefined>(undefined); const [viewMode, setViewMode] = useState<ViewMode>(getInitialViewMode); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null); const [modalError, setModalError] = useState<string | null>(null); const { confirm, confirmationDialog } = useConfirmDialog()
