...
export const DEFAULT_PLACE_FILTERS: PlaceFilters = {
  ...
  isFavorite: null, isVisited: null, ratingMin: null, sortBy: 'name', sortDirection: 'asc',
}
...
const queryKeys: Record<keyof PlaceFilters, string> = {
  ...
  isFavorite: 'favorite', isVisited: 'visited', ratingMin: 'rating_min', sortBy: 'sort', sortDirection: 'direction',
}
...
export function serializePlaceFilters(filters: PlaceFilters): URLSearchParams {
  ...
  for (const key of ['isFavorite', 'isVisited'] as const) if (value[key] !== null) params.set(queryKeys[key], String(value[key]))
...
export function deserializePlaceFilters(params: URLSearchParams): PlaceFilters {
  ...
  result.isFavorite = triState(params.get(queryKeys.isFavorite)); result.isVisited = triState(params.get(queryKeys.isVisited))
...
export function countActivePlaceFilters(filters: PlaceFilters): number {
  ...
  return Number(Boolean(value.query)) + listKeys.reduce((count, key) => count + Number(value[key].length > 0), 0)
    + ['hasPhotos', 'createdFrom', 'createdTo', 'updatedFrom', 'updatedTo', 'hasValidCoordinates', 'inTrip', 'isFavorite', 'isVisited', 'ratingMin'].reduce((count, key) => count + Number(value[key as keyof PlaceFilters] !== null && value[key as keyof PlaceFilters] !== ''), 0)
...
export function buildPlaceFilterSearchParams(filters: PlaceFilters): URLSearchParams {
  ...
  if (value.isVisited !== null) params.set('is_visited', String(value.isVisited))
