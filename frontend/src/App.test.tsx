...
vi.mock('./api/trips', () => ({ getTrip: vi.fn(), listTrips: vi.fn(), listAccessibleTrips: vi.fn(() => Promise.resolve([])), restoreTripState: vi.fn() }))
...
describe('map URL workspace', () => {
  ...
  it('routes mobile MAP_MODE Sorties to the current map trip catalog', async () => {
    vi.stubGlobal('matchMedia', vi.fn().mockImplementation((query: string) => ({ matches: query === MOBILE_NAVIGATION_MEDIA_QUERY, media: query, addEventListener: vi.fn(), removeEventListener: vi.fn() })))
    render(<MemoryRouter initialEntries={[`/maps/${MAP_ID}`]}><App /><Path /></MemoryRouter>)

    await screen.findByTestId('workspace')
    const mobileNavigation = await screen.findByRole('navigation', { name: 'Navigation CartaVault' })
    fireEvent.click(within(mobileNavigation).getByRole('button', { name: 'Sorties' }))

    await waitFor(() => expect(screen.getByTestId('path')).toHaveTextContent(`/maps/${MAP_ID}/trips`))
    expect(await screen.findByRole('heading', { name: 'Sorties de la carte' })).toBeVisible()
    expect(listTrips).toHaveBeenCalledWith(MAP_ID, expect.any(AbortSignal))
  })

  it('opens and returns from a map-scoped trip without changing its map', async () => {
    vi.stubGlobal('matchMedia', vi.fn().mockImplementation((query: string) => ({ matches: query === MOBILE_NAVIGATION_MEDIA_QUERY, media: query, addEventListener: vi.fn(), removeEventListener: vi.fn() })))
    vi.mocked(listTrips).mockResolvedValue([TRIP] as never)
    render(<MemoryRouter initialEntries={[`/maps/${MAP_ID}/trips`]}><App /><Path /></MemoryRouter>)

    fireEvent.click(await screen.findByRole('button', { name: 'Ouvrir' }))
    await waitFor(() => expect(screen.getByTestId('path')).toHaveTextContent(`/travels/trip-1?from_map=${MAP_ID}`))
    expect(await screen.findByRole('complementary', { name: 'Préparation de sortie' })).toBeVisible()
  ...
  it('rejects a trip that does not belong to the requested return map', async () => {
    vi.mocked(getMaps).mockResolvedValue([MAP, MAP_B])
    render(<MemoryRouter initialEntries={[`/travels/trip-1?from_map=${MAP_ID_B}`]}><App /><Path /></MemoryRouter>)

    await waitFor(() => expect(screen.getByTestId('path')).toHaveTextContent(`/maps/${MAP_ID_B}/trips`))
    expect(screen.queryByRole('complementary', { name: 'Préparation de sortie' })).not.toBeInTheDocument()
  })
