...
2. **Open PR limit** is capped (5 for app ecosystems, 3 for Docker) to keep the queue reviewable.
...
5. **Temporary ignores** go in `.github/dependabot.yml` with a short `reason` and a tracking issue. Remove the ignore when the blocker is fixed.
